...
// cpclib_basm::assemble() contains thread-unsafe global state (also guarded in
// cpclib-basm's own tests).  Serialize all calls that go through the assembler
// so tests generated by test_resources don't stomp on each other.
static ASM_LOCK: LazyLock<Mutex<()>> = LazyLock::new(|| Mutex::new(()));
...
/// Holds `ASM_LOCK` for the duration to serialise concurrent test threads.
fn assemble_source(source: &str) -> Result<Vec<u8>, String> {
    let _guard = ASM_LOCK.lock().unwrap_or_else(|e| e.into_inner());
