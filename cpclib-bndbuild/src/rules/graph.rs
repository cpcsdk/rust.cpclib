...
pub struct TaskTargetsForLayer<'a> {
    pub tasks: HashSet<TaskTargets<'a>>,
}

impl<'a> TaskTargetsForLayer<'a> {
    pub fn new(tasks: HashSet<TaskTargets<'a>>) -> Self {
        TaskTargetsForLayer { tasks }
    }
    pub fn iter(&self) -> std::collections::hash_set::Iter<'_, TaskTargets<'a>> {
        self.tasks.iter()
    }

    pub fn tasks(&self) -> &HashSet<TaskTargets<'a>> {
        &self.tasks
    }
...
pub struct TaskTargets<'a> {
    pub targets: HashSet<&'a Utf8Path>,
}

impl Hash for TaskTargets<'_> {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        let mut targets: Vec<&&Utf8Path> = self.targets.iter().collect();
        targets.sort();
...
impl<'a> TaskTargets<'a> {
    pub fn new(targets: HashSet<&'a Utf8Path>) -> Self {
        TaskTargets { targets }
    }
    pub fn iter(&self) -> std::collections::hash_set::Iter<'_, &'a Utf8Path> {
        self.targets.iter()
    }

    pub fn targets(&self) -> &HashSet<&'a Utf8Path> {
        &self.targets
    }
...
impl<'r> Graph<'r> {
    pub fn get_layered_dependencies(&self) -> LayeredDependenciesByTask<'r> {
        ...
        for layer in layers {
            let mut task_to_targets: BTreeMap<usize, HashSet<&Utf8Path>> = BTreeMap::new();
            for target in layer {
                if let Some(idx) = self.node2tracked.get(target) {
                    task_to_targets.entry(*idx).or_default().insert(target);
                }
            }
            if !task_to_targets.is_empty() {
                let tasks = task_to_targets.into_values().map(TaskTargets::new).collect();
        ...
        let orphans = self
            ...
            .map(|r| TaskTargets::new(r.targets().iter().map(|p| p.as_path()).collect::<HashSet<_>>()))
            .collect::<HashSet<_>>();
        if !orphans.is_empty() {
    ...
    pub fn get_layered_dependencies_for<P: AsRef<Utf8Path>>(
        ...
        LayeredDependenciesByTask { layers: res.into_iter().filter(|l| !l.is_empty()).collect_vec() }
    }
