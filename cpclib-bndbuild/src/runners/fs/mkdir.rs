...
impl<E: EventObserver> Runner for MkdirRunner<E> {
    ...
    fn inner_run<S: AsRef<str>>(&self, itr: &[S], o: &E) -> Result<(), String> {
        ...
        let args = MkdirArgs::from_arg_matches(&matches).map_err(|e| e.to_string())?;
...
mod test {
    ...
    fn test_mkdir_multiple_directories() {
        ...
        let mkdir = MkdirRunner::default();
        mkdir
            .inner_run(&[dir1.to_string(), dir2.to_string()], &())
            .unwrap();
    ...
    #[test]
    fn test_mkdir_version_flag_captured() {
        use cpclib_common::event::CapturingObserver;
        let mkdir = MkdirRunner::default();
        let obs = CapturingObserver::new();
        let result = mkdir.inner_run(&["--version"], &obs);
    ...
    #[test]
    fn test_mkdir_invalid_arg_captured() {
        use cpclib_common::event::CapturingObserver;
        let mkdir = MkdirRunner::default();
        let obs = CapturingObserver::new();
        let result = mkdir.inner_run(&["--not-a-valid-flag"], &obs);
