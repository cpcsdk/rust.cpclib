...
mod test {
    ...
    fn test_rm_multiple_files() {
        ...
        let rm = RmRunner::default();
        rm.inner_run(&[path1.to_string(), path2.to_string()], &())
            .unwrap();
    ...
    #[test]
    fn test_rm_help_flag_captured() {
        use cpclib_common::event::CapturingObserver;
        let rm = RmRunner::default();
        let obs = CapturingObserver::new();
        let result = rm.inner_run(&["--help"], &obs);

        assert!(result.is_ok(), "help should succeed");
        let out = obs.stdout_joined();
        assert!(!out.is_empty(), "help text should appear in observer stdout");
    ...
    #[test]
    fn test_rm_version_flag_captured() {
        use cpclib_common::event::CapturingObserver;
        let rm = RmRunner::default();
        let obs = CapturingObserver::new();
        let result = rm.inner_run(&["--version"], &obs);
    ...
    #[test]
    fn test_rm_invalid_arg_captured() {
        use cpclib_common::event::CapturingObserver;
        let rm = RmRunner::default();
        let obs = CapturingObserver::new();
        let result = rm.inner_run(&["--not-a-valid-flag"], &obs);
