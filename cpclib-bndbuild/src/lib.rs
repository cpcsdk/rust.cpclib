...
use task::FAP_CMDS;
use task::{
    ACE_CMDS, AMSPIRIT_CMDS, AMSPIRITLITE_CMDS, ARCHIVE_CMDS, ASMFMT_CMDS, AT_CMDS, AYT_CMDS,
    BASM_CMDS, BASMOPT_CMDS, BDASM_CMDS, BNDBUILD_CMDS, CADENCE_CMDS, CAPRICEFOREVER_CMDS,
    CHIPNSFX_CMDS, CONVGENERIC_CMDS, CP_CMDS, CPCEC_CMDS, CPCEMU_CMDS, CPCEMUPOWER_CMDS, CPR_CMDS,
    CSL_CMDS, DISARK_CMDS, DISC_CMDS, ECHO_CMDS, EMUCTRL_CMDS, EMULATOR_1984_CMDS, EXTERN_CMDS,
    HIDEUR_CMDS, HSPC_CMDS, HXCFE_CMDS, IMG2CPC_CMDS, IMPDISC_CMDS, MARTINE_CMDS, MKDIR_CMDS,
    MV_CMDS, ORGAMS_CMDS, RASM_CMDS, RETROVM_CMDS, RM_CMDS, SJASMPLUS_CMDS, SNA_CMDS,
    SONG2AKM_CMDS, SUGARBOX_CMDS, UZ80_CMDS, VASM_CMDS, VLINK_CMDS, WINAPE_CMDS, XFER_CMDS,
    Z80PROFILER_CMDS
};
use thiserror::Error;
...
/// Contains compilation timestamps, version info, and other build metadata.
pub mod built_info {
...
pub fn build_args_parser() -> clap::Command {
    ...
    let cmd = cmd.arg(
        ...
        .arg(
            Arg::new("graph_details")
                .long("graph-details")
                .help("Include detailed information in the dependency graph (requires --dot)")
                .action(ArgAction::SetTrue)
                .requires("dot")
        )
        .arg(
    ...
    cmd.arg(
        Arg::new("completion")
            .long("completion")
            .help("Generate autocompletion configuration. Optionally specify shell (bash, zsh, fish, etc.), otherwise auto-detects")
