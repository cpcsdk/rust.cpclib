...
use minus::{MinusError, Pager, page_all, ExitStrategy};
use std::fmt::Write;
use crate::*;
...
fn mem_to_string(sna: &Snapshot, from: Option<u32>, amount: Option<u32>) -> String {
	...
	let amount = amount.unwrap_or_else(||{
		sna.memory.len() as u32 - from
	});

	(from..(from+amount)).into_iter()
...
fn parse_help<'src>(input: Source<'src>) -> IResult<Source<'src>, Command, VerboseError<Source<'src>>> {
	map(
		tag_no_case("HELP"),
		|_| Command::Help
	)(input)
...
fn parse_load2<'src>(input: Source<'src>) -> IResult<Source<'src>, Command, VerboseError<Source<'src>>> {
	map(
			preceded(
				tuple((
					tag_no_case("LOAD2"),
			...
			,
			cut(context("Filename needs to be in a string", recognize(
				delimited(
					char('"'),
					take_until("\""),
					char('"')
...
pub fn cli( fname: &str, mut sna: Snapshot) {
    ...
    loop {
		let fname1 = std::path::Path::new(fname).file_name().unwrap().to_str().unwrap();
		let prompt = if let Some((fname2,_)) = &sna2 {
			let fname2 = std::path::Path::new(fname2).file_name().unwrap().to_str().unwrap();
