...
pub(crate) fn extract_symbols_from_token<T: ListingElement + std::fmt::Display>(
    token: &T
) -> Vec<String> {
    // Skip comments and label definitions (they're not references)
    if token.is_comment()
        || token.is_label()
        || token.is_macro_definition()
...
pub(crate) fn collect_references_in_content(
    ...
    let symbol_regexes: Vec<(&str, regex::Regex)> = symbol_names
        .iter()
        .filter_map(|&symbol| {
        ...
        for (symbol, re) in &symbol_regexes {
            if re.is_match(line) {
                ...
                let highlighted = highlight_cache
                    .entry(context_key)
                    .or_insert_with(|| {
                        Arc::from(syntax::link_symbols_in_source(&context, all_symbols).as_str())
                    })
                    .clone();
