use cpclib_basmdoc::{DocumentationPage, UndocumentedConfig};

#[test]
fn test_function_support() {
    // Test that the DocumentationPage has function support
    let page = DocumentationPage::for_file("../cpclib-asm/assets/ga.asm", "ga.asm", UndocumentedConfig::all()).expect("Failed to read ga.asm file");
