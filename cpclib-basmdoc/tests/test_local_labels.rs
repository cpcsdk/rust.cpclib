...
fn test_local_labels() {
    ...
    let label_names: Vec<String> = page
        .label_iter()
    ...
    assert!(
        label_names.contains(&"main_loop".to_string()),
        "main_loop should be present"
    );
    assert!(
        label_names.contains(&"process_data".to_string()),
        "process_data should be present"
    );

    // Check that local labels are prefixed with their parent
    assert!(
        label_names.contains(&"main_loop.loop".to_string()),
        "main_loop.loop should be present"
    );
    assert!(
        label_names.contains(&"process_data.inner".to_string()),
        "process_data.inner should be present"
    );

    // Check that orphan local label (before any global label) is NOT present
    assert!(
        !label_names.contains(&".orphan_local".to_string()),
        ".orphan_local should NOT be present (no parent)"
    );
    assert!(
        !label_names.iter().any(|l: &String| l.contains("orphan")),
        "No label should contain 'orphan'"
    );
    ...
    assert!(
        html.contains("main_loop.loop"),
        "HTML should contain main_loop.loop"
    );
}
