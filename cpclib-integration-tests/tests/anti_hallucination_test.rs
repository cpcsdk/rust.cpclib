                ...
                || (line_trimmed.contains(" -") && line_trimmed.contains("`-"))
            {
                ...
                for part in parts {
                    if part.starts_with("--") {
                        ...
                        if !opt.is_empty()
                            && opt.len() > 1
                            && !opt.chars().all(|c| c.is_whitespace())
                        {
                            options.insert(opt);
                        }
                    }
...
fn test_all_tools_for_hallucinations() {
    ...
    let tools: Vec<(&str, Command, &str)> = vec![
        ...
        (
            "locomotive",
            cpclib_locomotive::build_command(),
            "locomotive"
        ),
    ...
    if !all_hallucinations.is_empty() {
        ...
        panic!(
            "\n{} documentation hallucinations found! See above for details.",
            all_hallucinations.len()
        );
    }

    println!(
        "\n✓ No hallucinations detected across {} tools!",
        tested_count
    );
}
