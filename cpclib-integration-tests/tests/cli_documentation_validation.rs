...
fn extract_examples_from_markdown(content: &str) -> Vec<String> {
    ...
    for line in content.lines() {
        ...
        let is_known_command = KNOWN_TOOLS.iter().any(|tool| cleaned.starts_with(tool));
...
fn test_command_parsing(tool: &str, args: &[String]) -> Result<(), String> {
    let cmd = get_command_for_tool(tool).ok_or_else(|| format!("Unknown tool: {}", tool))?;
...
fn test_documentation_examples() {
    ...
    for md_file in md_files {
        let content = fs::read_to_string(&md_file).expect(&format!("Failed to read {:?}", md_file));
        ...
        for example in examples {
            ...
            if let Err(e) = test_command_parsing(&tool, &args) {
                failures.push((md_file.display().to_string(), example.clone(), e));
            }
        }
    }
...
fn test_bndbuild_commands_documented() {
    ...
    let content = fs::read_to_string(commands_md_path).expect("Failed to read commands.md");
    ...
    if !missing.is_empty() {
        ...
        panic!(
            "\n{} command aliases are not documented in commands.md",
            missing.len()
        );
    }
