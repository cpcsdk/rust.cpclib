	...
	fn dynamic_download_urls(&self) -> Result<MutiplatformUrls, String> {
		match self {
			Self::UnknownLastVersion => {
				...
				let macos = map.get("All versions").map(|url| format!("{}/{}", ACE_URL, url));
				let windows = map.get("x64 (64 bits)").map(|url| format!("{}/{}", ACE_URL, url));
				let linux = map.get("Ubuntu 22.04 LTS (AVX2)").map(|url| format!(
					"{}/{}",
					ACE_URL,
					url));
				...
				AceVersion::Bnd4 => Ok(MutiplatformUrls{
				...
				AceVersion::ZenSummer => Ok(MutiplatformUrls{
					linux: Some("http://www.roudoudou.com/ACE-DL/LinuxZenSummer.tar.gz".to_string()),
					windows: Some( "http://www.roudoudou.com/ACE-DL/Win64Summer.zip".to_string()),
					macos: None,
				}),
				AceVersion::WakePoint => Ok(MutiplatformUrls{
					linux: Some("http://www.roudoudou.com/ACE-DL/BZen.tar.gz".to_string()),
