...
//!
//! Shared by two callers: `cpclib-dap` (a DAP *server* that sits between an
//! editor and an emulator, translating on the way) and this crate's own
//! `Robot` automation (a DAP *client* that drives 1984js directly for
//! Orgams automation, with no editor in the loop at all). Both need the same
//! wire-format mechanics - message framing, response/event/request shapes -
//! so this lives here, in the crate both already depend on the same
//! direction (`cpclib-dap` → `cpclib-runner`), instead of being defined
//! twice. `cpclib-dap/src/protocol.rs` re-exports this module rather than
