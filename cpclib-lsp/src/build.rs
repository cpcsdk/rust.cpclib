...
impl BuildFileAnalyzer {
    ...
    fn validate_build_structure(&self, document: &Document) -> Vec<Diagnostic> {
        ...
        for line in text.lines() {
            ...
            let content = if trimmed.starts_with("- ") { trimmed[2..].trim_start() } else { trimmed };
            for &key in tgt_keys {
        ...
        for (line_num, line) in text.lines().enumerate() {
            ...
            for &key in dep_keys {
                ...
                if let Some(rest) = content.strip_prefix(prefix.as_str()) {
                    let value = rest.split('#').next().unwrap_or("").trim();
                    if value.starts_with('>') || value.starts_with('|') { break; }
                    ...
                    for tok in value.split_whitespace() {
                        ...
                        if !declared.contains(tok) {
                            ...
                            let file_exists = base_dir.as_ref()
    ...
    pub fn document_symbols(&self, document: &Document) -> Vec<DocumentSymbol> {
        ...
        let file_dir = document.uri.to_file_path().ok().and_then(|p| {
        ...
        let (expanded_text, source_map) =
            match crate::sourcemap::expand_with_source_map(&raw_text, file_dir.as_deref()) {
                ...
                Err(_) => {
                    ...
                    (raw_text.clone(), crate::sourcemap::SourceMap::identity(lines))
                }
            };
    ...
    fn scan_symbols_from_text(
        ...
        for (exp_idx, line_str) in text.lines().enumerate() {
            ...
            if trimmed.starts_with("- ") {
                ...
                self.process_key_value(rest, orig, target_names,
                    &mut rule_tgt, &mut rule_help,
                    &mut collecting, &mut block_base, &mut block_buf);
            } else if in_rule {
                rule_end = orig;
                self.process_key_value(trimmed, orig, target_names,
                    &mut rule_tgt, &mut rule_help,
                    &mut collecting, &mut block_base, &mut block_buf);
            }
        }
    ...
    fn process_key_value(
        ...
        if rule_tgt.is_none() && target_names.contains(&key) {
            match value {
                ...
                "" => {}
                v  => { *rule_tgt = Some((v.to_string(), orig)); }
            }
