...
fn test_assembly_completions_include_instructions() {
    ...
    assert!(
        completions.contains(&"ADD"),
        "Should include ADD instruction"
    );
    assert!(
        completions.contains(&"SUB"),
        "Should include SUB instruction"
    );
    assert!(completions.contains(&"JP"), "Should include JP instruction");
    assert!(
        completions.contains(&"CALL"),
        "Should include CALL instruction"
    );
    assert!(
        completions.contains(&"RET"),
        "Should include RET instruction"
    );
    assert!(
        completions.contains(&"NOP"),
        "Should include NOP instruction"
    );
}
...
fn test_assembly_completions_include_directives() {
    ...
    assert!(
        ...
        "Should include INCLUDE directive"
    );

    // Check block directives
    assert!(
        start.contains(&"MACRO"),
        "Should include MACRO start directive"
    );
...
fn test_assembly_data_non_empty() {
    ...
    assert!(
        !cpclib_asm::lsp::Z80_INSTRUCTIONS.is_empty(),
        "Instructions should not be empty"
    );
    assert!(
        !cpclib_asm::lsp::Z80_REGISTERS.is_empty(),
        "Registers should not be empty"
    );
    assert!(
        !cpclib_asm::lsp::ASSEMBLER_DIRECTIVES_STANDALONE.is_empty(),
        "Standalone directives should not be empty"
    );
}
...
fn test_common_instructions_present() {
    ...
    let common_instructions = [
        "LD", "ADD", "SUB", "AND", "OR", "XOR", "CP", "INC", "DEC", "PUSH", "POP", "CALL", "RET",
        "JP", "JR", "DJNZ", "NOP", "HALT", "EI", "DI", "IN", "OUT", "EX", "EXX", "RL", "RR", "SLA",
        "SRA"
    ];
...
fn test_common_registers_present() {
    ...
    let common_registers = ["AF", "BC", "DE", "HL", "IX", "IY", "IXL", "IXH"];
