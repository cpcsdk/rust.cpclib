    ...
    ($fn_name:ident, $keyword:expr, $token:ident, $constraint:expr, $error_msg:expr) => {
        ...
        pub fn $fn_name<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
            let (_, space, expr) = (
                ...
                parse_basic_space1,
                cut_err(
                    parse_numeric_expression($constraint).context(StrContext::Label($error_msg))
                )
            )
                .parse_next(input)?;
        ...
        match parse_instruction(input) {
            ...
            Err(_) => {
                    ...
                    .flatten()
                {
                    ...
                    tokens.push(BasicToken::CommentOrString(
                        BasicTokenNoPrefix::SymbolQuote,
                        comment_text.as_bytes().to_vec(),
                        false
                    ));
                }
                break;
...
pub fn parse_instruction<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    ...
    let mut instruction = alt((
        ...
        alt((
            ...
            alt((parse_new, parse_clear, parse_end, parse_stop))
        )),
        // Math/Misc & I/O
        alt((
            alt((
...
pub fn parse_string_variable<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    ...
    if let Some(_) = opt('(').parse_next(input)? {
        ...
        let mut first_index =
            parse_numeric_expression(NumericExpressionConstraint::None).parse_next(input)?;
        tokens.append(&mut first_index);

        // Additional indices
        while let Ok((mut comma, mut next_index)) = (
            parse_comma,
            parse_numeric_expression(NumericExpressionConstraint::None)
        )
            .parse_next(input)
...
pub fn parse_print<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    ...
    let mut tokens =
        construct_token_list!(BasicToken::SimpleToken(BasicTokenNoPrefix::Print), space);
...
pub fn parse_input<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    ...
    let (space_a, suppress_cr): (_, _) = (parse_basic_space0, opt(';')).parse_next(input)?;
    ...
    let (canal, mut space_after_canal): (_, _) =
        (opt(parse_canal), parse_basic_space0).parse_next(input)?;
    ...
    let (prompt_string, prompt_sep, mut space_after_prompt) =
        if let Ok(string) = parse_quoted_string(true).parse_next(input) {
    ...
    let mut res =
        construct_token_list!(BasicToken::SimpleToken(BasicTokenNoPrefix::Input), space_a);
...
pub fn parse_cls<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    let (_, space, canal) =
        (Caseless("CLS"), parse_basic_space0, opt(parse_canal)).parse_next(input)?;
...
pub fn parse_next<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    let (_, space, opt_var) =
        (Caseless("NEXT"), parse_basic_space0, opt(parse_variable)).parse_next(input)?;
...
pub fn parse_if<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
        ...
        parse_numeric_expression(NumericExpressionConstraint::Integer).parse_next(input)
    {
        ...
        if let Ok((mut else_space1, _, mut else_space2)) =
            (parse_basic_space0, Caseless("ELSE"), parse_basic_space0).parse_next(input)
        {
    ...
    if !is_eol_or_else {
        ...
        loop {
            ...
            let else_checkpoint = input.checkpoint();
            if opt((parse_basic_space0, Caseless::<&str>("ELSE")))
                .parse_next(input)?
                .is_some()
            {
                input.reset(&else_checkpoint);
...
pub fn parse_drawr<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    ...
    let (space_a, x, comma1, y, opt_params) = (
        ...
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::None)
                .context(StrContext::Label("X offset expected"))
        ),
        cut_err(parse_comma.context(StrContext::Label("Comma expected"))),
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::None)
                .context(StrContext::Label("Y offset expected"))
        ),
        opt((
            parse_comma,
            opt(parse_numeric_expression(
...
pub fn parse_dim<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    ...
    let mut parse_array_decl = |input: &mut &'src str| {
        ...
        let mut dim1 =
            parse_numeric_expression(NumericExpressionConstraint::None).parse_next(input)?;
        let mut extra_dims: Vec<Vec<BasicToken>> = repeat(
            0..,
            (
                parse_comma,
                parse_numeric_expression(NumericExpressionConstraint::None)
    ...
    let rest_arrays: Vec<Vec<BasicToken>> = repeat(
        0..,
        (parse_comma, parse_array_decl).map(|(mut c, mut arr)| {
            let mut tokens = vec![];
            tokens.append(&mut c);
...
pub fn parse_poke<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    let (_, space_a, address, comma, value) = (
        ...
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::Integer)
                .context(StrContext::Label("Value expected"))
        )
    )
        .parse_next(input)?;
...
pub fn parse_out<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    let (_, space_a, port, comma, value) = (
        ...
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::Integer)
                .context(StrContext::Label("Port expected"))
        ),
        cut_err(parse_comma.context(StrContext::Label("Comma expected"))),
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::Integer)
...
pub fn parse_sound<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    ...
    let (space_a, channel, comma1, period, comma2, duration) = (
        ...
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::Integer)
                .context(StrContext::Label("Channel expected"))
        ),
        cut_err(parse_comma.context(StrContext::Label("Comma expected"))),
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::Integer)
                .context(StrContext::Label("Period expected"))
        ),
        cut_err(parse_comma.context(StrContext::Label("Comma expected"))),
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::Integer)
                .context(StrContext::Label("Duration expected"))
        )
    )
        .parse_next(input)?;
...
pub fn parse_on_goto_gosub<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    let (_, space_a, expr, space_b) = (
        ...
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::None)
                .context(StrContext::Label("Expression expected"))
        ),
        parse_basic_space1
    )
        .parse_next(input)?;
    ...
    let mut res = construct_token_list!(
        ...
        expr,
        space_b,
        vec![BasicToken::SimpleToken(if is_goto {
...
pub fn parse_on_error_goto<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    ...
    Ok(construct_token_list!(
        BasicToken::SimpleToken(BasicTokenNoPrefix::On),
        space_a,
        vec![BasicToken::SimpleToken(BasicTokenNoPrefix::Error)],
        space_b,
        vec![BasicToken::SimpleToken(BasicTokenNoPrefix::Goto)],
        space_c,
        line_num
    ))
}
...
pub fn parse_on_break<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    ...
    Ok(construct_token_list!(
        BasicToken::SimpleToken(BasicTokenNoPrefix::On),
        space_a,
        vec![BasicToken::SimpleToken(BasicTokenNoPrefix::OnBreak)],
        space_b
    ))
}
...
pub fn parse_speed_ink<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    let (_, space_a, _, space_b, period1, comma, period2) = (
        ...
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::Integer)
                .context(StrContext::Label("Period1 expected"))
        ),
        cut_err(parse_comma.context(StrContext::Label("Comma expected"))),
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::Integer)
                .context(StrContext::Label("Period2 expected"))
        )
    )
        .parse_next(input)?;
    ...
    Ok(construct_token_list!(
        ...
        space_a,
        vec![BasicToken::SimpleToken(BasicTokenNoPrefix::Ink)],
        space_b,
        period1,
        comma,
        period2
...
pub fn parse_speed_write<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    let (_, space_a, _, space_b, speed) = (
        ...
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::Integer)
                .context(StrContext::Label("Speed expected"))
        )
    )
        .parse_next(input)?;
    ...
    Ok(construct_token_list!(
        BasicToken::SimpleToken(BasicTokenNoPrefix::Speed),
        space_a,
        vec![BasicToken::SimpleToken(BasicTokenNoPrefix::Write)],
        space_b,
        speed
    ))
}
...
pub fn parse_speed_key<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    let (_, space_a, _, space_b, delay, comma, period) = (
        ...
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::Integer)
                .context(StrContext::Label("Start delay expected"))
        ),
        cut_err(parse_comma.context(StrContext::Label("Comma expected"))),
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::Integer)
                .context(StrContext::Label("Repeat period expected"))
        )
    )
        .parse_next(input)?;
    ...
    Ok(construct_token_list!(
        BasicToken::SimpleToken(BasicTokenNoPrefix::Speed),
        space_a,
        vec![BasicToken::SimpleToken(BasicTokenNoPrefix::Key)],
        space_b,
        delay,
        comma,
        period
...
pub fn parse_every<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    ...
    let res = construct_token_list!(
        ...
        opt_timer,
        space2,
        vec![BasicToken::SimpleToken(BasicTokenNoPrefix::Gosub)],
        space3,
        line
    );
...
pub fn parse_after<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    ...
    let (space1, time) = (
        ...
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::Integer)
                .context(StrContext::Label("Time expected"))
        )
    )
        .parse_next(input)?;
...
pub fn parse_list<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    let (_, space) = (Caseless("LIST"), parse_basic_space0).parse_next(input)?;
...
pub fn parse_renum<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    let (_, space) = (Caseless("RENUM"), parse_basic_space0).parse_next(input)?;
...
pub fn parse_def_fn<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    ...
    let fn_name_tokens: Vec<BasicToken> = fn_name
        .chars()
    ...
    let mut res = construct_token_list!(
        ...
        space1,
        vec![BasicToken::SimpleToken(BasicTokenNoPrefix::Fn)],
        space2,
        fn_name_tokens
    );
...
pub fn parse_defint<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    ...
    let rest: Vec<Vec<BasicToken>> = repeat(
        0..,
        (parse_comma, parse_letter_range).map(|(mut c, mut r)| {
            let mut tokens = vec![];
            tokens.append(&mut c);
            tokens.append(&mut r);
            tokens
...
pub fn parse_symbol<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    ...
    let (space_a, char_code) = (
        ...
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::Integer)
                .context(StrContext::Label("Character code expected"))
        )
    )
        .parse_next(input)?;
    ...
    for _ in 0..8 {
        let (comma, row) = (
            ...
            cut_err(
                parse_numeric_expression(NumericExpressionConstraint::Integer)
                    .context(StrContext::Label("Row data expected"))
            )
        )
            .parse_next(input)?;
        rows.push((comma, row));
...
pub fn parse_symbol_after<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    ...
    let mut res =
        construct_token_list!(BasicToken::SimpleToken(BasicTokenNoPrefix::Symbol), space_a);
...
pub fn parse_tag<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    let (_, space, opt_canal) =
        (Caseless("TAG"), parse_basic_space0, opt(parse_canal)).parse_next(input)?;
...
pub fn parse_tagoff<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    let (_, space, opt_canal) =
        (Caseless("TAGOFF"), parse_basic_space0, opt(parse_canal)).parse_next(input)?;
...
pub fn parse_wait<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    let (_, space_a, port, comma1, mask, opt_inv) = (
        ...
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::Integer)
                .context(StrContext::Label("Mask expected"))
        ),
        opt((
            parse_comma,
            parse_numeric_expression(NumericExpressionConstraint::Integer)
...
pub fn parse_window<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    ...
    let (space_a, canal, left, comma1, right, comma2, top, comma3, bottom) = (
        ...
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::Integer)
                .context(StrContext::Label("Left expected"))
        ),
        cut_err(parse_comma.context(StrContext::Label("Comma expected"))),
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::Integer)
                .context(StrContext::Label("Right expected"))
        ),
        cut_err(parse_comma.context(StrContext::Label("Comma expected"))),
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::Integer)
                .context(StrContext::Label("Top expected"))
        ),
        cut_err(parse_comma.context(StrContext::Label("Comma expected"))),
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::Integer)
                .context(StrContext::Label("Bottom expected"))
        )
    )
        .parse_next(input)?;
...
pub fn parse_window_swap<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    let (_, space_a, _, space_b, stream1, comma, stream2) = (
        ...
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::Integer)
                .context(StrContext::Label("Stream1 expected"))
        ),
        cut_err(parse_comma.context(StrContext::Label("Comma expected"))),
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::Integer)
                .context(StrContext::Label("Stream2 expected"))
        )
    )
        .parse_next(input)?;
    ...
    Ok(construct_token_list!(
        BasicToken::SimpleToken(BasicTokenNoPrefix::Window),
        space_a,
        vec![BasicToken::SimpleToken(BasicTokenNoPrefix::Swap)],
        space_b,
        stream1,
        comma,
        stream2
...
pub fn parse_graphics_pen<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    ...
    Ok(construct_token_list!(
        ...
        space_a,
        vec![BasicToken::SimpleToken(BasicTokenNoPrefix::Pen)],
        space_b,
        opt_mode
    ))
}
...
pub fn parse_graphics_paper<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    ...
    Ok(construct_token_list!(
        BasicToken::SimpleToken(BasicTokenNoPrefix::Graphics),
        space_a,
        vec![BasicToken::SimpleToken(BasicTokenNoPrefix::Paper)],
        space_b,
        opt_mode
    ))
}
...
pub fn parse_origin<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    ...
    if let Some((mut c2, mut left, mut c3, mut right, mut c4, mut top, mut c5, mut bottom)) =
        opt_bounds
    {
        res.append(&mut c2);
        res.append(&mut left);
...
pub fn parse_line_input<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    ...
    let space_a = parse_basic_space1.parse_next(input)?;
    let _ = cut_err(Caseless("INPUT").context(StrContext::Label("INPUT expected")))
        .parse_next(input)?;
    let space_b = parse_basic_space0.parse_next(input)?;
    let canal = opt(parse_canal).parse_next(input)?;
    ...
    Ok(construct_token_list!(
        ...
        space_a,
        vec![BasicToken::SimpleToken(BasicTokenNoPrefix::Input)],
        space_b,
        canal,
        space_c,
        sep_token,
...
pub fn parse_key<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    let (_, space_a, key_num, comma) = (
        ...
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::Integer)
                .context(StrContext::Label("Key number expected"))
        ),
        cut_err(parse_comma.context(StrContext::Label("Comma expected")))
    )
        .parse_next(input)?;
...
pub fn parse_key_def<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    let (_, space_a, _, space_b, key_num, comma1, repeat, comma2, delay) = (
        ...
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::Integer)
                .context(StrContext::Label("Repeat expected"))
        ),
        cut_err(parse_comma.context(StrContext::Label("Comma expected"))),
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::Integer)
                .context(StrContext::Label("Delay expected"))
        )
    )
        .parse_next(input)?;
    ...
    Ok(construct_token_list!(
        BasicToken::SimpleToken(BasicTokenNoPrefix::Key),
        space_a,
        vec![BasicToken::SimpleToken(BasicTokenNoPrefix::Def)],
        space_b,
        key_num,
        comma1,
        repeat,
...
pub fn parse_zone<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    let (_, space, width) = (
        ...
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::Integer)
                .context(StrContext::Label("Width expected"))
        )
    )
        .parse_next(input)?;
...
pub fn parse_write<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    ...
    let (space, opt_canal_comma) =
        (parse_basic_space0, opt((parse_canal, parse_comma))).parse_next(input)?;
    ...
    let opt_values: Option<(Vec<BasicToken>, Vec<Vec<BasicToken>>)> = opt((
        ...
        repeat(
            0..,
            (parse_comma, parse_print_expression).map(|(mut c, mut expr)| {
                let mut tokens = vec![];
                tokens.append(&mut c);
...
pub fn parse_mid_assign<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    ...
    // String variable
    let var = cut_err(parse_string_variable.context(StrContext::Label("String variable expected")))
        .parse_next(input)?;
    ...
    // Start position
    let start_pos = cut_err(
        parse_numeric_expression(NumericExpressionConstraint::Integer)
            .context(StrContext::Label("Start position expected"))
    )
    .parse_next(input)?;
    ...
    let value =
        cut_err(parse_string_expression.context(StrContext::Label("String expression expected")))
            .parse_next(input)?;

    Ok(construct_token_list!(
        BasicToken::SimpleToken(BasicTokenNoPrefix::MidDollar),
        space1,
        vec![BasicToken::SimpleToken(open.into())],
        var,
        comma1,
        start_pos,
        opt_length,
        space2,
        vec![BasicToken::SimpleToken(close.into())],
        space3,
        vec![BasicToken::SimpleToken(equals.into())],
        space4,
        value
    ))
}
...
fn parse_parenthesized_numeric_expression<'src>(
    input: &mut &'src str
) -> BasicSeveralTokensResult<'src> {
    let (_, expr, _) = (
        '(',
...
fn parse_numeric_term<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    ...
    loop {
        ...
        let is_logical: ModalResult<_, ContextError<StrContext>> =
            peek(alt((Caseless("AND"), Caseless("OR"), Caseless("XOR")))).parse_next(input);
...
pub fn parse_string_expression<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    ...
    let concatenations: Vec<Vec<BasicToken>> = repeat(
        0..,
        (
            parse_basic_space0,
            '+',
...
fn parse_unary_operator(
    ...
    opt(alt((
        (Caseless("NOT"), alt((' ', '\t', '(')))
            .map(|_| BasicToken::SimpleToken(BasicTokenNoPrefix::Not)),
        ...
        loop {
            ...
            let op_result: ModalResult<Vec<BasicToken>, ContextError<StrContext>> = alt((
                alt((
                    ...
                    "<=".map(|_| {
                        vec![BasicToken::SimpleToken(BasicTokenNoPrefix::LessThanOrEqual)]
                    }),
                    ">=".map(|_| vec![BasicToken::SimpleToken(BasicTokenNoPrefix::GreaterOrEqual)]),
                    "<>".map(|_| vec![BasicToken::SimpleToken(BasicTokenNoPrefix::NotEqual)]),
                    "<".map(|_| vec![BasicToken::SimpleToken(BasicTokenNoPrefix::LessThan)]),
...
pub fn parse_all_generated_string_functions<'src>(
    input: &mut &'src str
) -> BasicSeveralTokensResult<'src> {
...
fn parse_mid_dollar<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    let (
        ...
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::None)
                .context(StrContext::Label("start position"))
        ),
        opt((
            parse_basic_space0,
            ',',
...
fn parse_bin_dollar<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    ...
    let mut expr = cut_err(
        parse_numeric_expression(NumericExpressionConstraint::None)
            .context(StrContext::Label("Number expected"))
    )
    .parse_next(input)?;
...
fn parse_string_dollar<'src>(input: &mut &'src str) -> BasicSeveralTokensResult<'src> {
    let (
        ...
        cut_err(
            parse_numeric_expression(NumericExpressionConstraint::None)
                .context(StrContext::Label("numeric expression"))
        ),
        parse_basic_space0,
        cut_err(','.context(StrContext::Label(","))),
        parse_basic_space0,
    ...
    result.push(BasicToken::SimpleToken(
        BasicTokenNoPrefix::CharOpenParenthesis
    ));
        ...
        let (code, mut space_a, open, mut expr1, comma, mut expr2, close) = (
            ...
            cut_err(
                parse_numeric_expression(constraint)
                    .context(StrContext::Label("Wrong first parameter"))
            ),
            cut_err(','.context(StrContext::Label("Missing ',' between arguments"))),
            cut_err(
                parse_numeric_expression(constraint)
                    .context(StrContext::Label("Wrong second parameter"))
            ),
            cut_err(')'.context(StrContext::Label("Missing ')'")))
        )
            .parse_next(input)?;
...
// Manual expansion for winnow v1 compatibility (no tuple > 10)
#[allow(clippy::needless_lifetimes)]
pub fn parse_all_generated_numeric_functions_any<'src>(
    input: &mut &'src str
) -> BasicSeveralTokensResult<'src> {
...
#[allow(clippy::needless_lifetimes)]
pub fn parse_all_generated_numeric_functions_any2<'src>(
    input: &mut &'src str
) -> BasicSeveralTokensResult<'src> {
    alt((
        parse_remain,
...
#[allow(clippy::needless_lifetimes)]
pub fn parse_all_generated_numeric_functions_int<'src>(
    input: &mut &'src str
) -> BasicSeveralTokensResult<'src> {
    alt((
        parse_derr,
...
pub fn parse_binary_value_16bits<'src>(input: &mut &'src str) -> BasicOneTokenResult<'src> {
    (opt('-'), preceded(Caseless("&x"), bin_u16_inner))
        .map(|(neg, val)| {
            let val = val as i16;
...
mod test {
    ...
    fn test_parse_numeric_expression() {
        let mut input = "x<0";
        let result =
            parse_numeric_expression(NumericExpressionConstraint::None).parse_next(&mut input);
    ...
    fn test_parse_graphics_pen() {
        ...
        assert!(
            result_instr.is_ok(),
            "GRAPHICS PEN c instruction should parse"
        );
        ...
        assert!(
            result2.is_ok(),
            "GRAPHICS PEN 1 via parse_instruction should parse"
        );
        ...
        assert!(
            result3.is_ok(),
            "GRAPHICS PEN c via parse_instruction should parse"
        );
    ...
    fn test_parse_if() {
        ...
        let result_unary = parse_numeric_expression(NumericExpressionConstraint::None)
            .parse_next(&mut input_unary);
