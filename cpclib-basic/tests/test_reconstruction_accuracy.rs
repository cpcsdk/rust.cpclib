    ...
    if !differences.is_empty() {
        ...
        for (original, reconstructed) in &differences {
            ...
            for i in 0..max_len.min(10) {
                ...
                let orig_char = orig_chars
                    .get(i)
                    .map(|c| format!("{:?}", c))
                    .unwrap_or_else(|| "EOF".to_string());
                let recon_char = recon_chars
                    .get(i)
                    .map(|c| format!("{:?}", c))
                    .unwrap_or_else(|| "EOF".to_string());
        ...
        println!(
            "Total: {} out of {} cases have reconstruction differences",
            differences.len(),
            total_cases
        );
        println!(
            "Note: Some differences may be acceptable (e.g., floating-point precision, whitespace normalization)"
        );
    }
    else {
