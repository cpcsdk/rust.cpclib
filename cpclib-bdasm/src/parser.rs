...
use cpclib_common::winnow::ascii::{Caseless, line_ending, space0, space1};
use cpclib_common::winnow::combinator::{alt, preceded, separated};
use cpclib_common::winnow::error::{ContextError, ErrMode, ParseError};
use cpclib_common::winnow::token::{literal, take_till, take_while};

use crate::control_file::{ControlDirective, ControlFile};
use crate::{BdAsmError, DataBlocString, Result};
...
pub fn parse_value_or_label(
    s: &str,
    labels: &HashMap<u16, Cow<str>>
...
fn parse_value_or_label_string<'i>(
    input: &mut &'i [u8]
) -> std::result::Result<&'i [u8], ErrMode<ContextError>> {
    take_while(1.., |c: u8| {
...
pub fn parse_data_bloc_string(
    input: &mut &[u8]
) -> std::result::Result<DataBlocString, ErrMode<ContextError>> {
    let inclusive_range = (
...
fn origin_directive(
    input: &mut &[u8]
) -> std::result::Result<ControlDirective, ErrMode<ContextError>> {
...
fn control_line(
    input: &mut &[u8]
) -> std::result::Result<Option<ControlDirective>, ErrMode<ContextError>> {
    alt((
        directive.map(Some),
...
pub fn parse_control_file(
    input: &mut &[u8]
) -> std::result::Result<Vec<ControlDirective>, ErrMode<ContextError>> {
    let lines: Vec<Option<ControlDirective>> =
        separated(0.., control_line, line_ending).parse_next(input)?;
...
pub fn load_control_file(path: &Utf8PathBuf) -> Result<ControlFile> {
    ...
    let directives = parse_control_file(&mut input).map_err(|e| {
        BdAsmError::ControlFile(format!("Failed to parse control file '{}': {}", path, e))
    })?;
