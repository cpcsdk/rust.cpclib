...
fn test_hello_world_roundtrip() {
    ...
    assert!(disasm_output.exists(), "Disassembly output should be created");
    
    // Step 3: Check disassembly contains expected instructions
    let disasm_content = fs::read_to_string(&disasm_output).unwrap();
    assert!(disasm_content.contains("ld hl") || disasm_content.contains("LD HL"), 
        "Should contain LD HL instruction");
    assert!(disasm_content.contains("call") || disasm_content.contains("CALL"), 
        "Should contain CALL instruction");
    assert!(disasm_content.contains("ret") || disasm_content.contains("RET"), 
...
fn test_control_file_roundtrip() {
    ...
    assert!(control_content.contains("origin") || control_content.contains("label"), 
...
fn test_cpc_string_detection() {
    ...
    assert!(control_content.contains("cpcstring"), 
...
fn test_data_bloc_handling() {
    ...
    assert!(content.contains("LD A") || content.contains("ld a"), 
...
fn test_interactive_control_file_edit() {
    ...
    assert!(output.exists(), "Output should be created from control file");
    ...
    assert!(output_content.contains("start"), "Output should contain the label 'start'");
}
...
fn test_save_and_load_control_workflow() {
    ...
    assert!(output2_content.to_lowercase().contains("defb") || output2_content.to_lowercase().contains("db"),
