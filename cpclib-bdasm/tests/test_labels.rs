...
fn test_label_generation() {
    ...
    let has_ld_hl_label = disasm_content.lines().any(|line| {
        line.to_uppercase().contains("LD HL") && line.contains("label_1211")
    });
    
    let has_call_with_label = disasm_content.lines().any(|line| {
        line.to_uppercase().contains("CALL") && line.contains("label_1207")
    });
    ...
    assert!(has_ld_hl_label, "LD HL should use label_1211 for the message address");
    assert!(has_call_with_label, "CALL should use label_1207");
    assert!(no_bb5a_label, "Should not create labels for external addresses like 0xbb5a");
}
