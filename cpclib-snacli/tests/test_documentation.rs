...
fn test_token_docs_match_actual_tokens() {
    ...
    let documented_tokens = vec![
        ...
        (
            "GA_ROMCFG",
            "Screen mode - actual name is GA_ROMCFG but doc calls it GA_SCR_MODE"
        ),
        (
            "GA_PEN",
            "Selected color - actual name is GA_PEN but doc calls it GA_COL_SELECTED"
        ),
        ("GA_PAL", "Palette entry"),
        (
            "GA_MULTIMODE",
            "Multi-configuration - actual name is GA_MULTIMODE but doc calls it GA_MULTICONFIG"
        ),
        ...
        (
            "PPI_CTL",
            "PPI control - actual name is PPI_CTL but doc calls it PPI_CONTROL"
        ),
    ];
    ...
    let documented_bases: HashSet<_> = documented_tokens.iter().map(|(name, _)| *name).collect();
    ...
    for flag in &actual_flag_names {
        if !documented_bases.contains(flag.as_str()) {
            ...
            if !is_subreg {
                eprintln!(
                    "Info: Flag '{}' exists but is not documented in cmdline.md",
                    flag
                );
            }
        }
...
fn test_documented_examples_use_valid_options() {
    ...
    if let Ok(content) = std::fs::read_to_string(examples_path) {
        ...
        println!(
            "✓ Verified {} example commands use valid options",
            commands.len()
        );
    }
    else {
