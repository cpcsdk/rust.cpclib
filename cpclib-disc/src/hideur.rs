...
use cpclib_common::{camino::{Utf8Path, Utf8PathBuf}, clap::{value_parser, Arg, ArgAction, ArgMatches, Command}, parse_value, winnow::{error::ContextError, Parser}};

use crate::{amsdos::{AmsdosFileType, AmsdosHeader, AmsdosFile, AmsdosFileName}};
...
impl Display for HideurError {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		match self {
			HideurError::IoError(e) => write!(f, "IO error: {}", e.to_string()),
		}
	}
}
...
pub fn hideur_handle(matches: &ArgMatches) -> Result<(), HideurError> {
		...
		let filename = {
			...
			let (filename, extension) = {
				let parts = complete_filename.file_name().unwrap().split('.').collect::<Vec<_>>();
			...
			let header = match ftype {
				AmsdosFileType::Binary => {
					...
					let load = matches
						...
						.as_bytes();
					let load = parse_value::<_, ContextError>.parse(load).expect("Wrong LOAD format") as u16;


					let exec = if let Some(exec) = matches
						.get_one::<String>("EXEC") {
						let exec = exec.as_bytes();
						parse_value::<_, ContextError>.parse(exec).expect("Wrong EXEC format")
						as u16
					} else {
						load
					};
