...
impl PicFileFormatHeader {
	pub fn from_buffer(buffer: &[u8]) -> Self {
		...
		let format_revision = buffer[i]; i+=1;
		let number_of_track = buffer[i]; i+=1;
		let number_of_side = buffer[i]; i+=1;
		let track_encoding = buffer[i]; i+=1;
		let bit_rate = buffer[i] as u16 + 256*(buffer[i+1] as u16); i+=2;
		let floppy_rpm  = buffer[i] as u16 + 256*(buffer[i+1] as u16); i+=2;
		let floppy_interface = FloppyInterface::n(buffer[i]).unwrap() ; i += 1;
		i += 1; // dnu
		let track_list_offset  = buffer[i] as u16 + 256*(buffer[i+1] as u16); i+=2;
		let write_allowed = buffer[i]; i+=1;
		let single_step = Step::n(buffer[i]).unwrap(); i+=1;
		let track0s0_altencoding = TrackAltEncoding::n(buffer[i]).unwrap() ; i += 1;
		let track0s0_encoding = TrackEncoding::n(buffer[i]).unwrap() ; i += 1;
		let track0s1_altencoding = TrackAltEncoding::n(buffer[i]).unwrap() ; i += 1;
		let track0s1_encoding = TrackEncoding::n(buffer[i]).unwrap() ; i += 1;
...
impl PicTrack {
	pub fn from_buffer(buffer: &[u8]) -> Self {
		...
		let mut i=0;
		let offset = buffer[i] as u16 + 256*(buffer[i+1] as u16); i+=2;
		let track_len = buffer[i] as u16 + 256*(buffer[i+1] as u16); i+=2;
	...
	pub fn from_buffer(buffer: &[u8], list: &PicTrackList) -> Self {
		let elems = list.list()
			...
			.map(|pt| {
				...
				TrackData::from_buffer(&buffer[offset as usize..(offset+allocated_size) as usize], &pt)
			})
			.collect_vec();
		TrackDataList(elems)
	}
...
impl Hfe {
	...
	pub fn from_buffer(buffer: &[u8]) -> Self {
		...
		let offset_lut = PicTrackList::from_buffer(&buffer[512 .. (512+offset_buffer_len)], header.number_of_track);

		let tracks_data = TrackDataList::from_buffer(&buffer[512+offset_buffer_len..], &offset_lut);
...
mod test {
	...
	#[test]
	fn load_hfe() {
		let hfe = Hfe::open("tests/MOODY.HFE");
	}
}