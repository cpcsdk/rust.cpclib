 ...
 * package.json, kept in exactly one place so the 30 per-emulator command
 ...
 */
const EMULATOR_MENU_ENTRIES: { id: string; label: string; dapId?: string }[] = [
    { id: 'ace', label: 'ACE-DL' },
 ...
 */
async function refreshEmulatorInstallContext(serverPath: string): Promise<void> {
    let entries: EmulatorEntry[];
    try {
        entries = await listEmulators(serverPath);
    }
...
export function registerDebugging(
    ...
    context.subscriptions.push(
        ...
        // The native "Run with.../Debug with..." context submenus
        // (package.json's `cpclib.runWithEmulatorMenu`/
        // `cpclib.debugWithEmulatorMenu`) - one command pair (installed/
        // needs-install icon) per `EMULATOR_MENU_ENTRIES` entry, so right-
        // clicking a `.sna`/`.dsk` reaches every compatible emulator without
        // the QuickPick step {@link debugWithEmulator}/{@link runWithEmulator}
        // need (reported live as an ergonomic problem: a context-menu click
        // that still ends in a QuickPick at the top of the screen might as
        // well have been the Command Palette). The install-state context
        // keys these `when` clauses read are refreshed once now and again
        // after every one of these fires, in case it just installed something.
