...
'`', 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o',
'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z', '{', '|', '}', '~', '␡', //TODO replace DEL by another char
' ', '▘', '▝', '▀', '▖', '▌', '▞', '▛', '▗', '▚', '▐', '▜', '▄', '▙', '▟', '█',
'·', '╵', '╶', '└', '╷', '│', '┌', '├', '╴', '┘', '─', '┴', '┐', '┤', '┬', '┼',
'^', '´', '¨', '£', '©', '¶', '§', '‘', '¼', '½', '¾', '±', '÷', '¬', '¿', '¡',
...
pub fn str_to_amscii(s: &str, charset: Charset) -> Result<Vec<u8>, String> {
	s.chars()
		.map(move |c| char_to_amscii(c, charset).ok_or_else(|| format!("{c} has no correspondance in the {} charset", charset.name())))
		.collect()
...
mod test {
	...
	fn test_english_string() {
		...
		assert_eq!(
			...
			Ok((0x20..=0x2F).into_iter().collect_vec())
		);

		assert_eq!(
			str_to_amscii("0123456789:;<=>?", Charset::English),
			Ok((0x30..=0x3F).into_iter().collect_vec())
		);

		assert_eq!(
			str_to_amscii("@ABCDEFGHIJKLMNO", Charset::English),
			Ok((0x40..=0x4F).into_iter().collect_vec())
		);

		assert_eq!(
			str_to_amscii("PQRSTUVWXYZ[\\]↑_", Charset::English),
			Ok((0x50..=0x5F).into_iter().collect_vec())
		);

		assert_eq!(
			str_to_amscii("`abcdefghijklmno", Charset::English),
			Ok((0x60..=0x6F).into_iter().collect_vec())
		);

		// here we skip 127/DEL
		assert_eq!(
			str_to_amscii("pqrstuvwxyz{|}~␡", Charset::English),
			Ok((0x70..=0x7f).into_iter().collect_vec())
		);
		...
		assert_eq!(
			str_to_amscii("▘▝▀▖▌▞▛▗▚▐▜▄▙▟█", Charset::English),
			Ok((0x81..=0x8F).into_iter().collect_vec())
		);

		assert_eq!(
			str_to_amscii("·╵╶└╷│┌├╴┘─┴┐┤┬┼", Charset::English),
			Ok((0x90..=0x9F).into_iter().collect_vec())
		);

		assert_eq!(
			str_to_amscii("^´¨£©¶§‘¼½¾±÷¬¿¡", Charset::English),
			Ok((0xa0..=0xaF).into_iter().collect_vec())
		);

		assert_eq!(
			str_to_amscii("αβγδεθλμπσφψχωΣΩ", Charset::English),
			Ok((0xb0..=0xbF).into_iter().collect_vec())
		);

		assert_eq!(
			str_to_amscii("🮠🮡🮣🮢🮧🮥🮦🮤🮨🮩🮮╳╱╲🮕▒", Charset::English),
			Ok((0xc0..=0xcF).into_iter().collect_vec())
		);

		assert_eq!(
			str_to_amscii("▔▕▁▏◤◥◢◣🮎🮍🮏🮌🮜🮝🮞🮟", Charset::English),
			Ok((0xd0..=0xdF).into_iter().collect_vec())
		);

		assert_eq!(
			str_to_amscii("☺☹♣♦♥♠○●□■♂♀♩♪☼🙭", Charset::English),
			Ok((0xe0..=0xef).into_iter().collect_vec())
		);

		assert_eq!(
			str_to_amscii("⭡⭣⭠⭢▲▼▶◀🯆🯅🯇🯈🙯🛧⭥⭤", Charset::English),
			Ok((0xf0..=0xff).into_iter().collect_vec())
		);
	}
}