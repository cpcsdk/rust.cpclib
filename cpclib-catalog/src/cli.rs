...
use cpclib_catart::interpret::Mode;
use log::error;
...
impl RenderOptions {
    ...
    pub fn parse_locale(&self) -> Locale {
        match self.locale.to_lowercase().as_str() {
            ...
            _ => {
                error!(
                    "Unknown locale '{}', defaulting to English. Valid options: english, french, spanish, german, danish",
                    self.locale
                );
                Locale::English
            }
    ...
    pub fn parse_mode(&self) -> Mode {
        match self.mode {
            ...
            _ => {
                error!(
                    "Invalid mode '{}', defaulting to Mode 1. Valid options: 0, 1, 2",
                    self.mode
                );
                Mode::Mode1
            }
