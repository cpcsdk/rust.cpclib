...
use cpclib_catart::{Locale, basic_command::BasicCommandList, char_command::CharCommandList, entry::{Catalog, CatalogType, PrintableEntry, PrintableEntryFileName, ScreenMode, SerialCatalogBuilder, UnifiedCatalog}, interpret::{Interpreter, Mode, display_screen_diff, screens_are_equal}};
use cpclib_common::num::Num;
use cpclib_disc::{AnyDisc, amsdos::{AmsdosEntries, AmsdosManagerNonMut, BlocIdx}, disc::Disc, edsk::Head, open_disc};
use log::{error, info};
...
pub fn handle_catalog_command(args: CatalogApp) -> Result<(), String> {
    match args.command {
        CatalogCommand::Build { basic_file, output_file, render_options } => {
