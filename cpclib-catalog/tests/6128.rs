...
fn test_6128_warmup() {
    ...
    let expected_screen_memory = std::fs::read("tests/6128.SCR").expect("Failed to read expected screen memory");


    if let Err(msg) = compare_memory_with_visual_diff(
        palette,
        (&expected_screen_memory[..]).try_into().expect("Expected screen memory must be exactly 16384 bytes"),
