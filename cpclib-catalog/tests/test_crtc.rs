...
use cpclib_catalog::{catalog_to_basic_listing, catalog_to_catart_commands, catalog_extraction};
use cpclib_catart::{
	basic_command::{BasicCommand, BasicCommandList},
	entry::{CatalogType, ScreenMode, UnifiedCatalog},
	interpret::{Interpreter, Locale, Mode},
};
...
fn test_crtc_catart() {
	...
	if orig_window_locate.len() != catalog_window_locate.len() {
		...
		eprintln!("Original has {} Window/Locate commands", orig_window_locate.len());
		eprintln!("Catalog has {} Window/Locate commands", catalog_window_locate.len());
		eprintln!("\nOriginal commands:");
		...
		panic!("Window/Locate command count mismatch: {} original vs {} catalog",
			orig_window_locate.len(), catalog_window_locate.len());
	}
	...
	for (index, (orig_cmd, catalog_cmd)) in orig_window_locate.iter().zip(catalog_window_locate.iter()).enumerate() {
		if orig_cmd != catalog_cmd {
			...
			eprintln!("Context:");
			if index > 0 {
				eprintln!("  [{}] Original: {:?}", index - 1, orig_window_locate[index - 1]);
				eprintln!("  [{}] Catalog:  {:?}", index - 1, catalog_window_locate[index - 1]);
			}
			...
			match (orig_cmd, catalog_cmd) {
				(BasicCommand::Window(ol, or, ot, ob), BasicCommand::Window(cl, cr, ct, cb)) => {
					...
					if ol != cl { eprintln!("  Left:   {} (orig) vs {} (catalog) — diff: {}", ol, cl, (*cl as i16) - (*ol as i16)); }
					if or != cr { eprintln!("  Right:  {} (orig) vs {} (catalog) — diff: {}", or, cr, (*cr as i16) - (*or as i16)); }
					if ot != ct { eprintln!("  Top:    {} (orig) vs {} (catalog) — diff: {}", ot, ct, (*ct as i16) - (*ot as i16)); }
					if ob != cb { eprintln!("  Bottom: {} (orig) vs {} (catalog) — diff: {}", ob, cb, (*cb as i16) - (*ob as i16)); }
				},
				(BasicCommand::Locate(ox, oy), BasicCommand::Locate(cx, cy)) => {
					eprintln!("Locate parameter differences:");
					if ox != cx { eprintln!("  Column: {} (orig) vs {} (catalog) — diff: {}", ox, cx, (*cx as i16) - (*ox as i16)); }
					if oy != cy { eprintln!("  Row:    {} (orig) vs {} (catalog) — diff: {}", oy, cy, (*cy as i16) - (*oy as i16)); }
				},
				_ => {
					eprintln!("Command type mismatch: original is {:?}, catalog is different type", orig_cmd);
				}
			}
	...
	let expected_screen_bytes = fs::read("tests/discs/crtc/CRTC.SCR")
		.expect("Failed to read CRTC.SCR file");

	eprintln!("Expected screen size: {} bytes", expected_screen_bytes.len());

	// Execute catalog BASIC program to generate screen memory
	let catalog_char_commands = catalog_basic_command_list.to_char_commands()
		.expect("Failed to convert catalog BASIC to CharCommandList");

	eprintln!("Total catalog CharCommands: {}", catalog_char_commands.iter().count());
	
	// Also run the original BASIC and compare
	eprintln!("\nExecuting ORIGINAL BASIC commands...");
	let orig_char_commands = orig_basic_command_list.to_char_commands()
		.expect("Failed to convert original BASIC to CharCommandList");
	
	eprintln!("Total original CharCommands: {}", orig_char_commands.iter().count());
	eprintln!("Orig Mode commands: {}", orig_char_commands.iter().filter(|c| matches!(c, cpclib_catart::char_command::CharCommand::Mode(..))).count());
	eprintln!("Orig Paper commands: {}", orig_char_commands.iter().filter(|c| matches!(c, cpclib_catart::char_command::CharCommand::Paper(..))).count());
	eprintln!("Orig Pen commands: {}", orig_char_commands.iter().filter(|c| matches!(c, cpclib_catart::char_command::CharCommand::Pen(..))).count());
	eprintln!("Orig Ink commands: {}", orig_char_commands.iter().filter(|c| matches!(c, cpclib_catart::char_command::CharCommand::Ink(..))).count());
	eprintln!("Orig Window commands: {}", orig_char_commands.iter().filter(|c| matches!(c, cpclib_catart::char_command::CharCommand::Window(..))).count());
	eprintln!("Orig Cls commands: {}", orig_char_commands.iter().filter(|c| matches!(c, cpclib_catart::char_command::CharCommand::Cls)).count());
	eprintln!("Orig Locate commands: {}", orig_char_commands.iter().filter(|c| matches!(c, cpclib_catart::char_command::CharCommand::Locate(..))).count());
	eprintln!("Orig Char commands: {}", orig_char_commands.iter().filter(|c| matches!(c, cpclib_catart::char_command::CharCommand::Char(..))).count());
	eprintln!("Orig Symbol commands: {}", orig_char_commands.iter().filter(|c| matches!(c, cpclib_catart::char_command::CharCommand::Symbol(..))).count());
	eprintln!("Orig PrintSymbol commands: {}", orig_char_commands.iter().filter(|c| matches!(c, cpclib_catart::char_command::CharCommand::PrintSymbol(..))).count());
	eprintln!("Orig String commands: {}", orig_char_commands.iter().filter(|c| matches!(c, cpclib_catart::char_command::CharCommand::String(..))).count());
	
	eprintln!("\nCatalog commands:");
	eprintln!("Mode commands: {}", catalog_char_commands.iter().filter(|c| matches!(c, cpclib_catart::char_command::CharCommand::Mode(..))).count());
	eprintln!("Paper commands: {}", catalog_char_commands.iter().filter(|c| matches!(c, cpclib_catart::char_command::CharCommand::Paper(..))).count());
	eprintln!("Pen commands: {}", catalog_char_commands.iter().filter(|c| matches!(c, cpclib_catart::char_command::CharCommand::Pen(..))).count());
	eprintln!("Ink commands: {}", catalog_char_commands.iter().filter(|c| matches!(c, cpclib_catart::char_command::CharCommand::Ink(..))).count());
	eprintln!("Window commands: {}", catalog_char_commands.iter().filter(|c| matches!(c, cpclib_catart::char_command::CharCommand::Window(..))).count());
	eprintln!("Cls commands: {}", catalog_char_commands.iter().filter(|c| matches!(c, cpclib_catart::char_command::CharCommand::Cls)).count());
	eprintln!("Locate commands: {}", catalog_char_commands.iter().filter(|c| matches!(c, cpclib_catart::char_command::CharCommand::Locate(..))).count());
	eprintln!("Char commands: {}", catalog_char_commands.iter().filter(|c| matches!(c, cpclib_catart::char_command::CharCommand::Char(..))).count());
	eprintln!("Symbol commands: {}", catalog_char_commands.iter().filter(|c| matches!(c, cpclib_catart::char_command::CharCommand::Symbol(..))).count());
	...
	let expected_screen_memory: [u8; 16384] = expected_screen_bytes.try_into()
...
fn test_crtc_two_paths_comparison() {
	...
	if path1_window_locate.len() != path2_window_locate.len() {
		panic!("Different number of commands: PATH1={}, PATH2={}", path1_window_locate.len(), path2_window_locate.len());
	}
	
	for (index, (cmd1, cmd2)) in path1_window_locate.iter().zip(path2_window_locate.iter()).enumerate() {
		if cmd1 != cmd2 {
	...
	eprintln!("\n✅ All {} Window/Locate commands match between both paths!", path1_window_locate.len());
}