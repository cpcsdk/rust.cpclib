...
pub fn compare_screens_with_visual_diff(
    ...
    if differences.is_empty() {
        ...
        for (addr, byte1, byte2) in differences.iter().take(20) {
            ...
            println!(
                "  Char position ({}, {}): screen1=0x{:02X}, screen2=0x{:02X} [addr: 0x{:04X}]",
...
fn generate_comparison_png_from_interpreters(
    ...
    let color_matrix1 = memory_screen1
    ...
    let color_matrix2 = memory_screen2
        .to_color_matrix_with_border(border_horizontal, border_vertical)
...
pub fn compare_memory_with_visual_diff(
    ...
    let img_expected = memory_to_image(
        expected,
        &mode,
        &palette,
        border_horizontal,
    ...
    let mut msg = format!(
        "❌ Screen memory mismatch: {} differences found\n",
        differences.len()
    );
    msg.push_str(&format!(
