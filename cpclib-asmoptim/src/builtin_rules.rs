...
mod tests {
    ...
    /// Which rules are actually executable today, by name - so implementing a
    /// constraint shows exactly which rules it unlocked rather than only
    /// moving a count.
    #[test]
    fn the_executable_builtin_rules_are_known() {
        assert_eq!(supported_names(OptimizationGoal::Neutral), vec![
            "neg-to-sub",
