...
const INSTRUCTIONS: &[&[u8]] = &[
    ...
    b"RRC", b"RRCA", b"RRD", b"RST", b"SBC", b"SCF", b"SET", b"SL1", b"SLA", b"SLL", b"SRA",
    b"SRL", b"SRL8", b"SUB", b"XOR"
];
...
pub fn generate_lsp_data() {
    let out_dir = env::var("OUT_DIR").unwrap();
    let dest_path = Path::new(&out_dir).join("lsp_data_generated.rs");
    let file = File::create(&dest_path).unwrap();
    let mut file = BufWriter::new(file);

    writeln!(
        file,
        "// Auto-generated by build.rs for LSP use - DO NOT EDIT"
    )
    .unwrap();
    writeln!(file).unwrap();

    // Generate Z80 instruction names as &str
    writeln!(file, "/// All Z80 instruction mnemonics").unwrap();
    writeln!(file, "pub const Z80_INSTRUCTIONS: &[&str] = &[").unwrap();
    for instruction in INSTRUCTIONS {
        let s = std::str::from_utf8(instruction).unwrap();
        writeln!(file, "    {:?},", s).unwrap();
    }
    writeln!(file, "];").unwrap();
    writeln!(file).unwrap();

    // Generate register names as &str
    writeln!(file, "/// All Z80 register names").unwrap();
    writeln!(file, "pub const Z80_REGISTERS: &[&str] = &[").unwrap();
    for register in REGISTERS {
        let s = std::str::from_utf8(register).unwrap();
        writeln!(file, "    {:?},", s).unwrap();
    }
    writeln!(file, "];").unwrap();
    writeln!(file).unwrap();

    // Generate assembler directive names as &str
    writeln!(file, "/// All assembler directives (standalone)").unwrap();
    writeln!(
        file,
        "pub const ASSEMBLER_DIRECTIVES_STANDALONE: &[&str] = &["
    )
    .unwrap();
    for directive in STAND_ALONE_DIRECTIVE {
        let s = std::str::from_utf8(directive).unwrap();
        writeln!(file, "    {:?},", s).unwrap();
    }
    writeln!(file, "];").unwrap();
    ...
    writeln!(file, "/// All assembler directives (start of block)").unwrap();
    writeln!(file, "pub const ASSEMBLER_DIRECTIVES_START: &[&str] = &[").unwrap();
    for directive in START_DIRECTIVE {
        let s = std::str::from_utf8(directive).unwrap();
        writeln!(file, "    {:?},", s).unwrap();
    }
    writeln!(file, "];").unwrap();
    ...
    writeln!(file, "/// All assembler directives (end of block)").unwrap();
    writeln!(file, "pub const ASSEMBLER_DIRECTIVES_END: &[&str] = &[").unwrap();
    for directive in END_DIRECTIVE {
        let s = std::str::from_utf8(directive).unwrap();
        writeln!(file, "    {:?},", s).unwrap();
    }
    writeln!(file, "];").unwrap();
