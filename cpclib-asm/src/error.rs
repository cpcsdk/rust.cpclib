    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            AssemblerError::IncludedFileError { span, error } => match error.as_ref() {
                ...
                _ => {
                    let msg =
                        build_simple_error_message("Error in imported file", span, Severity::Error);
                    write!(f, "{}", msg)?;
                    error.fmt(f)
                }
            },
            ...
            AssemblerError::OutputExceedsLimits(address, limit) => write!(
                f,
            ...
            AssemblerError::UnknownSymbol { symbol, closest } => write!(
                f,
                ...
                match error.deref() {
                    ...
                    AssemblerError::UnknownSymbol { symbol, closest } => {
                        let msg = match closest {
                            Some(closest) => build_simple_error_message_with_notes(
                                &format!("Unknown symbol: {}", symbol),
                                vec![format!("Closest one is: {}", closest)],
                                span,
                            ),
                            None => build_simple_error_message(
                                &format!("Unknown symbol: {}", symbol),
                                span,
                                Severity::Error,
                            ),
