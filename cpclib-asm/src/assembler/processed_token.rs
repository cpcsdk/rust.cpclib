...
pub struct ProcessedToken<'token, T: Visited + AsSimpleToken> {
...
enum ProcessedTokenState {
	...
	Include{listing: LocatedListing},
...
impl<'token, T: Visited + AsSimpleToken> From<&'token T> for ProcessedToken<'token, T> {
...
impl<'token, T:AsSimpleToken +  Visited> ProcessedToken<'token, T> {
    ...
    pub fn read_referenced_file(&mut self, env: &Env) -> Result<Option<AssemblerInfo>, AssemblerError> {
        ...
        self.state = match token {
            Token::Include(ref fname, _namespace, _once) => {
				...
				Some(ProcessedTokenState::Include{listing})
            }
...
impl<'token, T: Visited + AsSimpleToken>  ProcessedToken<'token, T> {
	...
	pub fn visited(&mut self, env: &mut Env) -> Result<(), AssemblerError> {
		match self.state {
			...
			Some(_) => {
				...
				unimplemented!("Specific behavior requiring a state not implemented")
			}
		}
	}
}
