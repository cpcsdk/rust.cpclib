...
impl ListingOutput {
    ...
    pub fn process_current_line(&mut self) {
        ...
        if !self.pending_iteration_notices.is_empty() && self.has_current_line_output() {
            if let Some(notice) = self.pending_iteration_notices.first().cloned() {
                self.render_raw_notice_line(&notice);
                self.pending_iteration_notices.remove(0);
            }
        }
...
endm\n";
    ...
    #[test]
    fn repeat_iteration_interleaves_counter_notice_with_generated_line() {
        let writer = SharedBufferWriter::default();
        ...
        assert!(first_counter < first_db, "listing={listing}");
        assert!(first_db < second_counter, "listing={listing}");
        assert!(second_counter < second_db, "listing={listing}");
    }
