...
impl Cruncher for CrunchType {
	fn crunch(&self, raw: &[u8]) -> Result<Vec<u8>, AssemblerError> {
		...
		match self {
			...
			CrunchType::LZ4 => {
				Err(AssemblerError::AssemblingError{msg:"LZ4 compression not implemented".to_owned()})
			}
			CrunchType::LZX7 => {
				Err(AssemblerError::AssemblingError{msg:"LZX7 compression not implemented".to_owned()})
			}
			CrunchType::LZEXO => {
				Err(AssemblerError::AssemblingError{msg:"LZEXO compression not implemented".to_owned()})
			}
			CrunchType::LZAPU => {
				Ok(crunchers::apultra::compress(raw))
			}
		}
	}		
}
