...
fn bench_parse_factor(c: &mut Criterion) {
    c.bench_with_input(BenchmarkId::new("parse_factor", "number"), &EXPR, |b, &expr| {
        b.iter(|| {
    ...
    c.bench_with_input(BenchmarkId::new("parse_factor", "label"), &LABEL_EXPR, |b, &expr| {
            let (_ctx, span) = ctx_and_span(expr);
        b.iter(|| {
            let span: cpclib_asm::Z80Span = span.clone();
            let span: cpclib_asm::InnerZ80Span = span.into();
    ...
    c.bench_with_input(BenchmarkId::new("parse_factor", "function"), &FUNC_EXPR, |b, &expr| {
            let (_ctx, span) = ctx_and_span(expr);
        b.iter(|| {
            let span: cpclib_asm::Z80Span = span.clone();
            let span: cpclib_asm::InnerZ80Span = span.into();
