...
//!
//! **The old published documents described this API inaccurately** - the
//! example client named endpoints that did not exist (`/api/ram_dump`,
//! `/api/pause`), and the old `web_api.md` omitted breakpoints, stepping,
//! `/api/history` and `/api/memmap` entirely. 1.14.3 adds a real,
//! authoritative alternative: `GET /api/doc`/`GET /api/doc/<name>`,
//! generated by the server itself rather than hand-maintained. Every mapping
//! below was cross-checked against it on the 1.13.4 → 1.14.3 upgrade and
//! matches exactly (request shape, body/query encoding, response fields) -
//! before that it was checked against a running 1.13.4 instance and the UI
//! the emulator itself serves, since nothing better existed yet. The
//! `live_tests` module keeps those checks runnable against whichever version
...
pub fn call_for(request: &Value) -> Option<Call> {
    ...
    Some(match command {
        ...
        // itself. The request shape (raw-text, comma-separated decimal line
        // numbers, `""` clears all) is confirmed exactly right by
        // `GET /api/doc/basic_bp` as of 1.14.3. What it does *not* mean is
        // that a pause always follows a real match: a live session showed
        // the armed set left silently unconsulted during a plain `continue`
        // (the program ran to completion, breakpoint never hit, no error),
        // reproduced again independently against this same 1.14.3 install.
        // `BasicSession`'s own `Continue` therefore does not trust this
        // mechanism either any more - it drives `cpclib/basicStep` in a loop
        // and decides the stop itself, the same way the generic path always
        // has. This still arms the set (harmless, and this emulator's own
        // web UI still shows it), just not as the thing anything here
        // waits on.
...
///
/// Ignored by default - they are how the mappings were verified against a
/// real instance (1.13.4 originally, re-run clean against 1.14.3 on that
/// upgrade with no changes needed), and are worth keeping runnable rather
/// than deleting once the machine goes away. Start one and run
...
mod live_tests {
    ...
    ///
    /// Region 0 (`#0000`) is not necessarily RAM: a machine at rest has lower
    /// ROM paged in there by default, and a region under ROM carries
    /// `rom_bank` instead of `ram_bank` (a real 1.14.3 observation, not a
    /// bug - `physical_of` already handles it, returning `None` for a ROM
    /// address before ever looking for `ram_bank`). So this checks the
    /// discriminated shape on every region, rather than assuming region 0
    /// specifically is RAM.
    #[test]
    #[ignore]
    fn live_memmap_names_the_page_at_each_region() {
        ...
        for region in regions {
            let is_rom = region.get("rom").and_then(Value::as_bool) == Some(true);
            let key = if is_rom { "rom_bank" } else { "ram_bank" };
            assert!(region.get(key).is_some(), "missing {key} in {region}");
        }
        assert!(
            map.get("rmr").is_some(),
