...
pub fn apply_cli_overrides(base: AsmFormatOptions, matches: &clap::ArgMatches) -> AsmFormatOptions {
    ...
    AsmFormatOptions {
        indent_size: if explicit("indent-size") {
            cli.indent_size
        }
        else {
            base.indent_size
        },
        comment_column: if explicit("comment-column") {
            cli.comment_column
        }
        else {
            base.comment_column
        },
        mnemonic_case: if explicit("mnemonic-case") {
            cli.mnemonic_case
        }
        else {
            base.mnemonic_case
        },
        directive_case: if explicit("directive-case") {
            cli.directive_case
        }
        else {
            base.directive_case
        },
        register_case: if explicit("register-case") {
            cli.register_case
        }
        else {
            base.register_case
        },
        one_instruction_per_line: if explicit("one-instruction-per-line") {
            cli.one_instruction_per_line
        }
        else {
            base.one_instruction_per_line
        },
        space_around_column: if explicit("space-around-column") {
            cli.space_around_column
        }
        else {
            base.space_around_column
        },
        space_around_assignment: if explicit("space-around-assignment") {
            cli.space_around_assignment
        }
        else {
            base.space_around_assignment
        },
        hexadecimal_case: if explicit("hexadecimal-case") {
            cli.hexadecimal_case
        }
        else {
            base.hexadecimal_case
        },
        hexadecimal_encoding: if explicit("hexadecimal-encoding") {
            cli.hexadecimal_encoding
        }
        else {
            base.hexadecimal_encoding
        },
        octal_encoding: if explicit("octal-encoding") {
            cli.octal_encoding
        }
        else {
            base.octal_encoding
        },
        binary_encoding: if explicit("binary-encoding") {
            cli.binary_encoding
        }
        else {
            base.binary_encoding
        },
        label_definition_postfix_with_column: if explicit("label-definition-postfix-with-column") {
