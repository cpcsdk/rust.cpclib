...
//! which a project may export only in part. The per-line detail comes from
//! the assembler's source map - the structured form of the listing, which
//! knows far more than a symbol table: for every emitted run its file,
//! line, columns, byte count and whether it is code or data, with macro
//! expansions recorded as contexts of their own. That is what lets bytes be
//! attributed to a source file, to a macro (the cost of its call sites), or
//! to one source line. The crunched cost of a region is measured, not
//! estimated: the whole image is crunched, then again with that region
//! removed, and the difference is the region's contribution. It is what a
//! size-limited intro actually needs - raw size and crunched size can rank
//! routines very differently.
//!
//! Originally built inside `cpclib-mcp`'s `size_map` tool; moved here so it
//! can be used directly - a CLI, a script, another tool - without an MCP
//! host in the loop. `cpclib-mcp` now calls straight into this module and
//! serializes [`SizeMapReport`] to JSON for its response.
...
use crate::resolve::{CRUNCHER_TIMEOUT, compress_with_timeout, resolve_cruncher};
...
#[derive(Debug, Default)]
pub struct MacroIndex(std::collections::HashMap<String, Vec<(u32, u32, String)>>);
...
/// What [`size_map`] takes beyond the entry path itself - all optional.
#[derive(Debug, Clone, Default)]
pub struct SizeMapOptions {
    ...
    /// [`crate::resolve::ALL_FORMATS`] name, e.g. `shrinkler`.
...
fn assemble_for_map(path: &Utf8Path, options: &SizeMapOptions) -> Result<Assembled, String> {
    ...
    cpclib_basmopt::apply_defines(&mut assemble, &options.defines)?;
    ...
    let opts = EnvOptions::new(parse, assemble, Arc::new(()));
    ...
    let (processed, env) = match visit_tokens_all_passes_with_options(&listing, opts) {
        Ok(done) => done,
...
pub struct SectionRegionCost {
    pub label: String,
    pub address: u32,
    pub bytes: u32,
    pub crunched_cost: Option<i64>
}

/// One crunched section (an `LZ48`/`LZSHRINKLER`/... block), measured from
/// the inside - see [`measure_sections`].
#[derive(Debug, Clone, serde::Serialize)]
pub struct CrunchedSectionReport {
    pub index: usize,
    pub cruncher: String,
    pub address: u32,
    pub decrunched_bytes: usize,
    pub crunched_bytes: usize,
    pub remeasured_crunched_bytes: Option<i64>,
    pub regions_table: String,
    pub lines_table: String,
    pub regions: Vec<SectionRegionCost>
}
...
/// One region as reported at the top level (raw bytes, not crunched-section
/// internals - see [`CrunchedSectionReport`] for those).
#[derive(Debug, Clone, serde::Serialize)]
...
/// The source-map breakdown by file/macro/source line - see [`attribute`].
#[derive(Debug, Clone, serde::Serialize)]
pub struct ListingDetail {
    pub files_table: String,
    pub macros_table: String,
    pub lines_table: String,
...
/// [`size_map`]'s result.
#[derive(Debug, Clone, serde::Serialize)]
pub struct SizeMapReport {
    pub listing: Option<ListingDetail>,
    pub crunched_sections: Vec<CrunchedSectionReport>,
    pub path: String,
    pub start: u32,
    pub end: u32,
    pub image_bytes: u32,
    pub cruncher: Option<String>,
    pub crunched_image: Option<i64>,
    /// The biggest regions as a ready-to-display markdown table.
    pub table: String,
    pub regions: Vec<RegionReport>
}

const LISTING_NOTE: &str = "Tables count image bytes by the source line that produced them. A crunched \
...
/// Assembles `path` (as a build would - give `options.include_dirs`/
/// `options.defines` matching the real build) and reports where its bytes
/// go: the biggest global labels (address, bytes to the next global label,
/// share of the image, local-label count), and - from the assembler's
/// source map - a code/data split per region plus three more tables: bytes
/// by source file, by macro (what a macro's call sites cost together), and
/// by individual source line. With `options.cruncher` set, each region also
/// gets a *measured* crunched cost (the whole image crunched, minus the
/// image without that region), and every crunched section in the source
/// (`LZ48`/`LZSHRINKLER`/...) gets its own breakdown of what went into it -
/// see [`CrunchedSectionReport`].
///
/// Labels come from the assembler's own symbol table (real addresses only,
/// never `equ` constants), not a `.sym` file. Spans include any data placed
/// between two labels; `SAVE` directives are never executed (dry run).
pub fn size_map(path: &Utf8Path, options: &SizeMapOptions) -> Result<SizeMapReport, String> {
    if let Some(c) = &options.cruncher {
        resolve_cruncher(c)?;
    }
    ...
    Ok(SizeMapReport {
        ...
        image_bytes: total,
        cruncher: options.cruncher.clone(),
...
mod tests {
    ...
    fn a_real_source_is_mapped_and_costed() {
        ...
        assert!(out.table.contains("filler"));
        assert!(out.crunched_image.unwrap() > 0);
    }
    ...
    fn regions_inside_a_crunched_section_are_costed_on_what_went_in() {
        ...
        .unwrap();
        let out = size_map(&path, &SizeMapOptions::default()).expect("size_map should succeed");
        let sections = &out.crunched_sections;
    ...
    fn a_real_macro_and_data_are_attributed_from_the_source_map() {
        ...
        fs_err::write(&path, "macro CLEAR\n ld a,0\n ld (hl),a\nendm\n org 0x4000\nfirst:\n CLEAR(void)\n CLEAR(void)\n ret\ntable:\n db 1,2,3,4,5\n").unwrap();
        let out = size_map(&path, &SizeMapOptions::default()).expect("size_map should succeed");
        let table = out.regions.iter().find(|r| r.label == "table").unwrap();
        assert_eq!((table.code_bytes, table.data_bytes), (Some(0), Some(5)));
