...
mod tests {
	...
	pub fn sna_various() {
		...
		assert_eq!(
			sna1.memory_dump().len(),
			64*1024
		);
		assert_eq!(
			sna1.memory_size_header(),
			0
		);
		...
		assert_eq!(
			sna1.memory_dump().iter().map(|v|{(*v) as usize}).sum::<usize>(),
			fixed.memory_dump().iter().map(|v|{(*v) as usize}).sum::<usize>()
		);
		assert_eq!(
			fixed.memory_size_header(),
			0
		);
		...
		assert_eq!(
			v2.memory_dump().iter().map(|v|{(*v) as usize}).sum::<usize>(),
			fixed.memory_dump().iter().map(|v|{(*v) as usize}).sum::<usize>()
		);

		// Save to reload (directly from V2)
		let tmp_fname = "/tmp/test.sna"; // TODO really use a tmp file
		v2.save(tmp_fname, SnapshotVersion::V2).expect("Unable to save");

		let sna2 = Snapshot::load(tmp_fname);
		assert_eq!(2, sna2.version_header());
		assert_eq!(
			sna1.memory_dump().len(),
			sna2.memory_dump().len()
		);

		assert_eq!(
			sna1.memory_dump().iter().map(|v|{(*v) as usize}).sum::<usize>(),
			sna2.memory_dump().iter().map(|v|{(*v) as usize}).sum::<usize>()
		);

		// Should also work from V3
		let tmp_fname = "/tmp/test.sna"; // TODO really use a tmp file
		sna1.save(tmp_fname, SnapshotVersion::V2).expect("Unable to save");
