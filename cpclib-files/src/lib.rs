...
impl FileAndSupport {
    ...
    pub fn new_amsdos<P: Into<Utf8PathBuf>>(p: P) -> Self {
        Self { support: StorageSupport::Host, file: (FileType::AmsdosBin, p.into()) }
    }

    pub fn new_amsdos_in_disc<P: Into<Utf8PathBuf>, F: Into<Utf8PathBuf>>(p: P, f: F) -> Self {
        Self { support: StorageSupport::Disc(p.into()), file: (FileType::AmsdosBin, f.into()) }
    }

    pub fn new_basic<P: Into<Utf8PathBuf>>(p: P) -> Self {
        Self { support: StorageSupport::Host, file: (FileType::AmsdosBas, p.into()) }
    }

    pub fn new_basic_in_disc<P: Into<Utf8PathBuf>, F: Into<Utf8PathBuf>>(p: P, f: F) -> Self {
        Self { support: StorageSupport::Disc(p.into()), file: (FileType::AmsdosBas, f.into()) }
    }

    pub fn new_ascii<P: Into<Utf8PathBuf>>(p: P) -> Self {
        Self { support: StorageSupport::Host, file: (FileType::Ascii, p.into()) }
    }

    pub fn new_ascii_in_disc<P: Into<Utf8PathBuf>, F: Into<Utf8PathBuf>>(p: P, f: F) -> Self {
        Self { support: StorageSupport::Disc(p.into()), file: (FileType::Ascii, f.into()) }
    }

    pub fn new_no_header<P: Into<Utf8PathBuf>>(p: P) -> Self {
        Self { support: StorageSupport::Host, file: (FileType::NoHeader, p.into()) }
    }
    ...
    fn build_amsdos_bin_file(&self, data: &[u8], loading_address: Option<u16>, exec_address: Option<u16>) -> Result<AmsdosFile, AmsdosError>{
        let size = data.len();
        if size > 0x10000 {
        ...
        let execution_address = exec_address
            .map(|e| if e < loading_address + size {e} else {loading_address})
    ...
    fn build_ascii_file(&self, data: &[u8]) -> Result<AmsdosFile, AmsdosError> {
        match AmsdosFileName::try_from(self.amsdos_filename().as_str()) {
            Ok(amsfname) => Ok(AmsdosFile::ascii_file_from_buffer_with_name(&amsfname, data)),
            Err(e) => {
                if self.in_disc() {
                    Err(e)?;
                }
    ...
    pub fn build_file<'d>(&self, 
        data: &'d [u8], 
        loading_address: Option<u16>, exec_address: Option<u16>
        ...
        match self.resolve_file_type() {
            ...
            FileType::AmsdosBas => {
                self.build_amsdos_bas_file(data)
                .map(Either::Left)
            },
            FileType::Ascii => {
                self.build_ascii_file(data)
                .map(Either::Left)
            },
            FileType::NoHeader => {
                ...
                let (fname, content) = match &built_file {
                    Either::Left(amsdos_file) => {
                        if self.resolve_file_type() == FileType::Ascii {
                            ...
                            let fname = amsdos_file.amsdos_filename().unwrap().unwrap().ibm_filename();
                            (fname, amsdos_file.header_and_content())
                        }
                    },
                    Either::Right(buffer) => {
                        (self.filename().into(), *buffer)
                    },
                };

                std::fs::write(&fname, content).map_err(
                    |e| {
                       format!(
                                "Error while saving \"{}\". {}",
                                fname,
