...
use cpclib_catart::entry::{CatalogType, ScreenMode, UnifiedCatalog, Catalog, PrintableEntry, PrintableEntryFileName};
...
fn make_fname(name: &str) -> PrintableEntryFileName {
    ...
    let extension = if parts.len() > 1 { parts[1].as_bytes() } else { &[] };
...
fn test_simple_entries_distribution() {
    ...
    let mode1_grid = unified_catalog.visible_entries_by_mode_and_order(ScreenMode::Mode1, CatalogType::Cat);
    
    println!("Grid dimensions: {} columns, {} max rows", 
    ...
    let displayed: Vec<String> = mode1_grid.entries_display_order()
        .map(|entry| {
            let fname_bytes = entry.fname().all_generated_bytes();
    ...
    let sorted_names: Vec<String> = sorted_for_check.iter()
    ...
    let sort_correct = sorted_names.iter()
    ...
    println!("Actual display: {:?}", displayed.iter().map(|s| s.trim()).collect::<Vec<_>>());
    ...
    for (i, (actual, expected)) in displayed.iter().zip(expected_display.iter()).enumerate() {
        ...
        if actual_trimmed != *expected {
            println!("❌ Position {}: Got '{}', Expected '{}'", i, actual_trimmed, expected);
            all_match = false;
        } else {
        ...
        if non_empty > 5 {
            ...
            for i in 0..64 {
                ...
                if status != 0xE5 {
                    ...
                    let as_chars: String = name_bytes.iter()
                        .map(|&b| {
                            if b >= 32 && b < 127 {
                                format!("{}", b as char)
                            } else {
                ...
                .map(|i| {
                    ...
                    PrintableEntry {
                        ...
                        fname: PrintableEntryFileName {
                            f1: entry_bytes[1], f2: entry_bytes[2], f3: entry_bytes[3], f4: entry_bytes[4],
                            f5: entry_bytes[5], f6: entry_bytes[6], f7: entry_bytes[7], f8: entry_bytes[8],
                            e1: entry_bytes[9], e2: entry_bytes[10], e3: entry_bytes[11],
                        },
                        pieces: [entry_bytes[12], entry_bytes[13], entry_bytes[14], entry_bytes[15]],
                        sectors: entry_bytes[16..32].try_into().unwrap(),
                    }
                })
                .collect();
            
            let catalog = Catalog::try_from(entries_vec.as_slice()).expect("Failed to create catalog");
            let unified_catalog = UnifiedCatalog::from(catalog);
            ...
            println!("Entries with user bytes like 0x21, 0x22, 0x23... (!, \", #...) are sequentially");
            println!("numbered in disk order, which suggests they should be displayed in that order");
...
fn test_sorting_with_control_codes() {
    ...
    entries.push(PrintableEntry {
        user: 0,
        fname: PrintableEntryFileName {
            f1: 0x06, f2: 0x1F, f3: 0x01, f4: 0x01, // Control codes
            f5: 0x0F, f6: 0x01, f7: b'Z', f8: 0x17,
            e1: b'.', e2: b'T', e3: b'X',
        },
        pieces: [0; 4],
        sectors: [0; 16],
    ...
    entries.push(PrintableEntry {
        user: 0,
        fname: PrintableEntryFileName {
            f1: 0x06, f2: 0x1F, f3: 0x01, f4: 0x01,
            f5: 0x0F, f6: 0x01, f7: b'A', f8: 0x17,
            e1: b'.', e2: b'T', e3: b'X',
        },
        pieces: [0; 4],
        sectors: [0; 16],
    ...
    entries.push(PrintableEntry {
        ...
        fname: PrintableEntryFileName {
            f1: 0x06, f2: 0x1F, f3: 0x01, f4: 0x01,
            f5: 0x0F, f6: 0x01, f7: b'M', f8: 0x17,
            e1: b'.', e2: b'T', e3: b'X',
        },
        pieces: [0; 4],
        sectors: [0; 16],
    ...
    for (i, entry) in entries.iter().enumerate() {
        ...
        println!("   Display char position: f7 = '{}' (0x{:02X})", bytes[6] as char, bytes[6]);
    }
    ...
    println!("\n--- Analysis ---");
    println!("When sorting by raw bytes (comparing [0x06, 0x1F, 0x01, 0x01, 0x0F, 0x01, f7, ...]):");
...
fn test_cat_vs_dir_filling_logic() {
    ...
    let cat_grid = unified_catalog.visible_entries_by_mode_and_order(ScreenMode::Mode1, CatalogType::Cat);
    
    println!("Column 0: {:?}", 
        cat_grid.column(0).unwrap().iter()
            .map(|e| e.fname().display_name())
            .collect::<Vec<_>>()
    );
    println!("Column 1: {:?}", 
        cat_grid.column(1).unwrap().iter()
            .map(|e| e.fname().display_name())
            .collect::<Vec<_>>()
    );
    
    let cat_display: Vec<String> = cat_grid.entries_display_order()
        .map(|e| e.fname().display_name())
        .collect();
    ...
    let dir_grid = unified_catalog.visible_entries_by_mode_and_order(ScreenMode::Mode1, CatalogType::Dir);
    
    println!("Column 0: {:?}", 
        dir_grid.column(0).unwrap().iter()
            .map(|e| e.fname().display_name())
            .collect::<Vec<_>>()
    );
    println!("Column 1: {:?}", 
        dir_grid.column(1).unwrap().iter()
            .map(|e| e.fname().display_name())
            .collect::<Vec<_>>()
    );
    
    let dir_display: Vec<String> = dir_grid.entries_display_order()
        .map(|e| e.fname().display_name())
        .collect();
