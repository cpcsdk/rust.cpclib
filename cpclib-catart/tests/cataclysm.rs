...
fn test_cataclysm_triplet(test_name: &str, mode: u8) {
    ...
    interpreter
        .interpret(char_commands.as_slice(), true)
    ...
    if differences.is_empty() {
        ...
        for (addr, expected, actual) in differences.iter().take(20) {
            ...
            println!(
                "  Char position ({}, {}): expected 0x{:02X}, got 0x{:02X} [addr: 0x{:04X}]",
...
fn generate_comparison_png(
    ...
    let original_img = image::open(original_png_path).unwrap_or_else(|e| {
        panic!(
            "Failed to load original PNG {}: {}",
            original_png_path.display(),
            e
        )
    });
    ...
    let color_matrix = memory_screen
        .to_color_matrix_with_border(border_horizontal, border_vertical)
    ...
    let mut diff_screen =
        BasicMemoryScreen::new_with_locale(mode, palette_diff.clone(), Locale::English);
    ...
    let diff_color_matrix = diff_screen
        .to_color_matrix_with_border(border_horizontal, border_vertical)
    ...
    comparison_img.save(&output_path).unwrap_or_else(|e| {
        panic!(
            "Failed to save comparison PNG to {}: {}",
            output_path.display(),
            e
        )
    });
