use cpclib_basic::BasicProgram;
use cpclib_catart::basic_command::BasicCommandList;
use cpclib_catart::interpret::Interpreter;

const BASIC_CODE: &str = r#"10 MODE 2
15 BORDER 0
20 PAPER 0
30 INK 0,0
40 INK 1,26
50 LOCATE 13,6
60 PRINT "   ___  _____  _________  _______________________  _  __"
70 LOCATE 13,7
80 PRINT "  / _ // __/ |/ / __/ _ \/  _/ ___/_  __/  _/ __ \/ |/ /"
90 LOCATE 13,8
100 PRINT " / _  / _//    / _// // // // /__  / / _/ // /_/ /    /"
110 LOCATE 13,9
120 PRINT "/____/___/_/|_/___/____/___/\___/ /_/ /___/\____/_/|_/"
130 LOCATE 33,13
140 PRINT "proudly presents"
150 LOCATE 32,17
160 PRINT "HORNY  BYTE  LOVERS"
170 WINDOW 37,48,19,21
180 LOCATE 37,20
190 PRINT "RUN";
200 PRINT CHR$(34);
210 PRINT "HBL";
220 PRINT CHR$(34);
230 WINDOW 37,48,20,21"#;

fn main() {
    // Parse BASIC and convert to commands
    let program = BasicProgram::parse(BASIC_CODE).expect("Tokenization failed");
    let commands = cpclib_catart::convert::basic_to_commands(&program).expect("Conversion failed");
    let cmds = BasicCommandList::from(commands);

    let mut interpreter = Interpreter::new_6128();
    let char_cmds = cmds.to_char_commands().expect("Char conversion failed");
    interpreter.interpret(char_cmds.as_slice(), true);

    println!("--- Voxy Facehugger BASIC Output ---");
    println!("{}", interpreter);
}
