...
impl Catalog {
    ...
    pub fn extract_basic_from_sequential_catart(&self, _show_headers: bool) -> BasicProgram {
        ...
        let basic_str = entries
            ...
            .map(|cmds| cmds.into_iter().flat_map(|c| c.bytes())) // convert to the stream of bytes
...
impl<'cat> EntriesGrid<'cat> {
    ...
    pub fn commands_with_params(&self, show_headers: bool) -> CharCommandList {
        ...
        for row_entries in self.rows() {
            for entry in row_entries.flatten() {
...
impl EntryKind {
    ...
    pub fn bytes_for_commands(&self, cmds: &[CharCommand]) -> SmallVec<[u8; 16]> {
        ...
        let mut cmds_and_bytes = cmds.iter().zip(cmds_bytes);
            ...
            EntryKind::UnsortedHeterogeneousAdditionalColor(_dot_hiding) => {
                EntryConstraint {
                    slots: smallvec![
                        ...
                        EntryConstraintItem::Any(dot_help[1])
                    ]
                }
            },
        }
...
impl SerialCatalogBuilder {
    ...
    pub fn build(&self, commands: &CharCommandList, start_mode: ScreenMode) -> UnifiedCatalog {
        if commands.is_empty() {
            UnifiedCatalog::empty()
        }
        else {
            let entries = self.build_entries(commands, start_mode);
        ...
        while cmd_idx < commands.len() {
            ...
            if create_entry || force_nops {
                ...
                let entry = if entries.is_empty() {
                    ...
                    other_entry_kinds
                        .build_entry(&commands_buffer, Some(entries.len() as u8 + b' '))
                };
