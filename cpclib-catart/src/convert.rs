...
use cpclib_basic::{BasicLine, BasicProgram, tokens::{BasicToken, BasicTokenNoPrefix, BasicTokenPrefixed, BasicValue}};
...
fn consume_integer_argument<'b>(iter: &mut Vec<(u16, &'b BasicToken)>) -> Result<u8, CatArtError> {
    ...
    while eat_space {
        let res  = iter.pop().ok_or(CatArtError::NotEnoughTokens("Unexpected end of tokens".to_string()))?;
        pair = Some(res);
        match res.1 {
    ...
    match curr {
        BasicToken::Constant(_, value) => {
            match value.as_integer() {
                ...
                None => Err(CatArtError::InvalidParameter(line, "An integer value is expected".to_string())),
            }
        },
        _ => {
            return Err(CatArtError::IncompatibleBasicCommand(line, "Expected integer argument".to_string()));
        }
    }
} else {
        Err(CatArtError::NotEnoughTokens("Unexpected end of tokens".to_string()))
    }
...
fn consume_comma<'b>(iter: &mut Vec<(u16, &'b BasicToken)>) -> Result<(), CatArtError> {
    ...
    loop {
        let res = iter.pop().ok_or(CatArtError::NotEnoughTokens("Expected comma".to_string()))?;
        match res.1 {
            BasicToken::SimpleToken(BasicTokenNoPrefix::CharSpace) => {
                // continue eating spaces
            },
            ...
            _ => {
                return Err(CatArtError::IncompatibleBasicCommand(res.0, "Expected comma".to_string()));
            }
        }
    }
}
...
fn consume_ink_arguments<'b>(iter: &mut Vec<(u16, &'b BasicToken)>) -> Result<(u8, u8, Option<u8>), CatArtError> {
    ...
    consume_spaces(iter);
    let has_third_arg = matches!(iter.last(), Some((_, BasicToken::SimpleToken(BasicTokenNoPrefix::CharComma))));
...
fn consume_locate_arguments<'b>(iter: &mut Vec<(u16, &'b BasicToken)>) -> Result<(u8, u8), CatArtError> {
...
fn consume_window_arguments<'b>(iter: &mut Vec<(u16, &'b BasicToken)>) -> Result<(u8, u8, u8, u8), CatArtError> {
...
fn consume_print_statement<'b>(iter: &mut Vec<(u16, &'b BasicToken)>) -> Result<(PrintArgument, PrintTerminator), CatArtError> {
    ...
    loop {
        ...
        match pair.1 {
            ...
            BasicToken::SimpleToken(BasicTokenNoPrefix::ValueQuotedString) => {
                ...
                loop {
                    match iter.last() {
                       Some((_, BasicToken::SimpleToken(BasicTokenNoPrefix::ValueQuotedString))) => {
            ...
            BasicToken::PrefixedToken(BasicTokenPrefixed::ChrDollar) => {
                ...
                consume_spaces(iter);
                let next = iter.pop().ok_or(CatArtError::NotEnoughTokens("Expected ( after CHR$".to_string()))?;
                if !matches!(next.1, BasicToken::SimpleToken(BasicTokenNoPrefix::CharOpenParenthesis)) {
                    return Err(CatArtError::IncompatibleBasicCommand(next.0, "Expected ( after CHR$".to_string()));
                }
                ...
                consume_spaces(iter);
                let next = iter.pop().ok_or(CatArtError::NotEnoughTokens("Expected ) after CHR$ argument".to_string()))?;
                if !matches!(next.1, BasicToken::SimpleToken(BasicTokenNoPrefix::CharCloseParenthesis)) {
                    return Err(CatArtError::IncompatibleBasicCommand(next.0, "Expected ) after CHR$ argument".to_string()));
                }
    ...
    let terminator = if has_trailing_semicolon { PrintTerminator::None } else { PrintTerminator::CrLf };
...
pub fn basic_to_commands<'b>(basic: &'b BasicProgram) -> Result<BasicCommandList, CatArtError> {
    ...
    let mut line_token_pairs: Vec<(u16, &'b BasicToken)> = basic.lines().iter().map(|line: &BasicLine| {
        let nb = line.line_number();
        line.tokens().iter().map(move |t| (nb, t))
    }).flatten().rev().collect::<Vec<_>>();
    ...
    while  let Some((line, token)) = line_token_pairs.pop() {
        match token {
            ...
            BasicToken::SimpleToken(BasicTokenNoPrefix::Border) => {
                ...
                let ink2 = if let Some((_, BasicToken::SimpleToken(BasicTokenNoPrefix::CharComma))) = line_token_pairs.last() {
            ...
            _ => {
                return Err(CatArtError::IncompatibleBasicCommand(line, format!("Unsupported BASIC command token: {:?}", token)));
...
mod test {
    ...
    fn test_print() {
        ...
        assert_eq!(
            result,
            vec![
                BasicCommand::print_string_crlf(br#"HELLO"#.to_vec())
            ]
            .into()
        );
        ...
        assert_eq!(
            result,
            vec![
                BasicCommand::print_string(br#"HELLO"#.to_vec())
            ]
            .into()
        );
