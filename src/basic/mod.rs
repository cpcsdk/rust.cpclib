...
impl BasicProgram {
	...
	fn line_idx_as_valid_index(&self, idx: BasicProgramLineIdx) -> Result<BasicProgramLineIdx, BasicError> {
		match &idx {
		...
		if ! self.has_line(idx.clone()) {
			...
			match self.previous_idx(idx.clone()) {
				Some(previous_idx) => {
					...
					self.get_line_mut(previous_idx).unwrap()
...
mod test {
	...
	fn parse_correct() {
		...
		let bytes = prog.as_bytes();
		let expected = vec![
			10, 0, 10, 0, 131,  32,  28,   0x34,   0x12, 0,   
			10, 0, 20, 0, 131,  32,  28,   0x34,   0x12, 0,   
			0,   0, 0];
	...
	fn indices() {
		...
		assert_eq!(
			Err(BasicError::UnknownLine{idx:BasicProgramLineIdx::Index(2)}),
		...
		assert_eq!(
			None,
			prog.previous_idx(BasicProgramLineIdx::Index(0))
		);
	...
	fn hide2() {
		...
		let bytes = prog.as_bytes();
		let expected = vec![
			20, 0, 10, 0, 131,  32,  28,   0x34,   0x12, 0,   
			00, 0, 20, 0, 131,  32,  28,   0x34,   0x12, 0,   
			0,   0, 0];
